$(function(){
	...
	$("#userName_add_txt").blur(function(){
		...
		if(userName != null && userName != ""){
			$.post("/finance/settlement/allotOrderManage/queryUser.json", { userName: userName },
				function(data){
					if(data == null){
	...
	var gridcomplete = function(){
		...
		$(".supplierName_class").click(function(){
			...
			$.get("/finance/settlement/allotOrderManage/searchSupplier/" + id + ".htm",
					function(data){
						art.dialog({
							title: "供应商详情",
						    content: data,
		...
		$("#update_allot_form").ajaxForm({
			type: 'post',
			url:  '/finance/settlement/allotOrderManage/update.json',
		...
		$(".delete_allot").click(function(){
			...
			art.dialog({
	    	    ...
	    	    okValue: '确认',
	    	    ok: function () {
	    	    	$.post("/finance/settlement/allotOrderManage/delete.json", { supplierId: supplierId },
						function(data){
	...
	$("#updateBatch").click(function(){
		...
		if(ids.length > 0){
			...
			$("#update_batch_form").ajaxForm({
				type: 'post',
				url:  '/finance/settlement/allotOrderManage/updateBatch.json',
				success: function(data) {
					art.dialog.get("add-batch-dialog").close();
					$("#search_button").click();
				}
		...
		if(ids.length > 0){
			...
			art.dialog({
	    	    ...
	    	    ok: function () {
	    	    	...
	    	    	$.post("/finance/settlement/allotOrderManage/deleteBatch.json", { supplierIds: params },
						function(data){
