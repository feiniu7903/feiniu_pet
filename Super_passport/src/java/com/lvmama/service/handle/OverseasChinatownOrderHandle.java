 ...
 *
 */
public class OverseasChinatownOrderHandle implements CheckStockHandle{
	
	private static final Log log = LogFactory.getLog(OverseasChinatownOrderHandle.class);
	...
	public List<Item> check(BuyInfo buyinfo,List<Item> list) {
		log.info("OverseasChinatownOrderHandle check");
		for (Item item : list) {
			try {
				...
				if(getContract(buyinfo)!=null && StringUtils.isNotBlank(getContract(buyinfo).getMobile())){
					...
					String checkTicketBuyUrl = WebServiceConstant.getProperties("overseasChinatown.url")+"checkGoodsBuy/";
					...
					if (checkTicketBuyResult.startsWith(HttpsUtil.HTTP_ERROR_PREFIX)) {
						...
						if(SUCCESS_CODE.equals(checkTicketRspCode)){
							...
							log.info("overseasChinatownOrderHandle message: " + rspDesc);
							item.setStock(SupplierProductInfo.STOCK.LACK);
