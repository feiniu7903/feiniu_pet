 ...
 *
 */
public class HarbinIceAndSnowWorldProcessorImpl implements ApplyCodeProcessor, DestroyCodeProcessor {
	private OrderService orderServiceProxy = (OrderService) SpringBeanProxy.getBean("orderServiceProxy");
	private static final Log LOG = LogFactory.getLog(HarbinIceAndSnowWorldProcessorImpl.class);
	private static final String SUCCESS = "0";
	@Override
	public Passport apply(PassCode passCode) {
		LOG.info("HarbinIceAndSnowWorld Apply Code: " + passCode.getSerialNo());
		long startTime = 0L;
		...
		if (extId == null || "".equals(extId)) {
			try {
				...
				LOG.info("HarbinIceAndSnowWorld Apply serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				LOG.info("HarbinIceAndSnowWorld xmlResponse:" + xmlResponse);
				...
				if (code.equals(SUCCESS)) {
					...
					LOG.info("HarbinIceAndSnowWorld Apply Error message:" + description);
				}
			} catch (Exception e) {
				LOG.error("HarbinIceAndSnowWorld Apply serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
				passport.setComLogContent(e.getMessage());
				this.reapplySet(passport, passCode.getReapplyCount());
				LOG.error("HarbinIceAndSnowWorld Apply Error message:", e);
			}
		}else{
			sendImg(passport,request,passCode);
		}
	...
	public Passport destroy(PassCode passCode) {
		LOG.info("HarbinIceAndSnowWorld Destoy Code: " + passCode.getSerialNo());
		long startTime=0L;
		Passport passport = new Passport();
		...
		try {
			...
			LOG.info("HarbinIceAndSnowWorld Destoy serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			...
			if (response.getCode().equals(SUCCESS)) {
				...
				LOG.info("HarbinIceAndSnowWorld Destoy Success message:" + description);
			} else {
				...
				LOG.info("HarbinIceAndSnowWorld Destoy Error message:" + description);
			}
		} catch (Exception e) {
			LOG.error("HarbinIceAndSnowWorld destroy serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			passport.setStatus(PassportConstant.PASSCODE_STATUS.FAILED.name());
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.DESTROY.name());
			passport.setComLogContent(e.getMessage());
			LOG.error("HarbinIceAndSnowWorld Destoy Exception:", e);
		}
		return passport;
	}
	/**
	...
	private void sendImg(Passport passport,PWBRequest request,PassCode passCode){
		try {
			...
			if (imagebt != null) {
				...
				passport.setComLogContent("HarbinIceAndSnowWorldProcessorImpl_sendImg_Reading image error");
				passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
				this.reapplySet(passport, passCode.getReapplyCount());
			}
		} catch (Exception e) {
	...
	private byte[] getImageCode(PWBRequest request) throws Exception {
		...
		try {
			...
			LOG.error("HarbinIceAndSnowWorld getImage Error message:", e);
			throw e;
		}
		return imageByte;
	}
