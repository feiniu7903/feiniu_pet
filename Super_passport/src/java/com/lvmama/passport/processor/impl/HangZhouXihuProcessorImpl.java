 ...
 *
 */
public class HangZhouXihuProcessorImpl implements ApplyCodeProcessor, DestroyCodeProcessor {
	private OrderService orderServiceProxy = (OrderService) SpringBeanProxy.getBean("orderServiceProxy");
	private static final Log LOG = LogFactory.getLog(HangZhouXihuProcessorImpl.class);
	private static final String SUCCESS = "0";
	@Override
	public Passport apply(PassCode passCode) {
		LOG.info("HangZhouXihu Apply Code: " + passCode.getSerialNo());
		...
		if (extId == null || "".equals(extId)) {
			...
			try {
				...
				LOG.info("HangZhouXihu Apply serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				LOG.info("HangZhouXihu xmlResponse:" + xmlResponse);
				...
				if (code.equals(SUCCESS)) {
					...
					LOG.info("HangZhouXihu Apply Error message:" + description);
				}
			} catch (Exception e) {
				LOG.error("HangZhouXihu Apply serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
				passport.setComLogContent(e.getMessage());
				this.reapplySet(passport, passCode.getReapplyCount());
				LOG.error("HangZhouXihu Apply Exception message:", e);
			}
		}else{
			sendImg(passport,request,passCode);
		}
	...
	public Passport destroy(PassCode passCode) {
		LOG.info("HangZhouXihu Destroy Code: " + passCode.getSerialNo());
		Passport passport = new Passport();
		...
		try {
			...
			LOG.info("HangZhouXihu destroy serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			...
			if (response.getCode().equals(SUCCESS)) {
				...
				LOG.info("HangZhouXihu Destroy Success message:" + description);
			} else {
				...
				LOG.info("HangZhouXihu Destroy Error message:" + description);
			}
		} catch (Exception e) {
			LOG.error("HangZhouXihu destroy serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			passport.setStatus(PassportConstant.PASSCODE_STATUS.FAILED.name());
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.DESTROY.name());
			passport.setComLogContent(e.getMessage());
			LOG.error("HangZhouXihu Destroy Exception:", e);
		}
		return passport;
	}
	/**
	...
	private void sendImg(Passport passport,PWBRequest request,PassCode passCode){
		...
		if (imagebt != null) {
			...
			passport.setComLogContent("HangZhouXihuProcessorImpl_sendImg_Reading image error");
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
			this.reapplySet(passport, passCode.getReapplyCount());
		}
	}
	...
	private byte[] getImageCode(PWBRequest request) {
		...
		try {
			...
			LOG.error("HangZhouXihu getImage Exception message:", e);
		}
		return imageByte;
	}
