 ...
 *【在线支付产品对接智游宝系统】
 */
public class YanCheng_OnLineProcessorImpl implements ApplyCodeProcessor, DestroyCodeProcessor {
	private OrderService orderServiceProxy = (OrderService) SpringBeanProxy.getBean("orderServiceProxy");
	private static final Log LOG = LogFactory.getLog(YanCheng_OnLineProcessorImpl.class);
	private static final String SUCCESS = "0";
	@Override
	public Passport apply(PassCode passCode) {
		LOG.info("YanCheng Apply Code: " + passCode.getSerialNo());
		...
		if (extId == null || "".equals(extId)) {
			...
			try {
				...
				LOG.info("YanCheng xmlResponse:" + xmlResponse);
				...
				if (code.equals(SUCCESS)) {
					...
					LOG.info("YanCheng Apply Error message:" + description);
				}
			} catch (Exception e) {
				...
				this.reapplySet(passport, passCode.getReapplyCount());
				LOG.error("YanCheng Apply Exception message:", e);
			}
		}else{
			sendImg(passport,request,passCode);
		}
	...
	public Passport destroy(PassCode passCode) {
		LOG.info("YanCheng Destroy Code: " + passCode.getSerialNo());
		Passport passport = new Passport();
		...
		try {
			...
			LOG.info("YanCheng destroy serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			...
			if (response.getCode().equals(SUCCESS)) {
				...
				LOG.info("YanCheng Destroy Success message:" + description);
			} else {
				...
				LOG.info("YanCheng Destroy Error message:" + description);
			}
		} catch (Exception e) {
			LOG.error("YanCheng destroy serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			passport.setStatus(PassportConstant.PASSCODE_STATUS.FAILED.name());
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.DESTROY.name());
			passport.setComLogContent(e.getMessage());
			LOG.error("YanCheng Destroy Exception:", e);
		}
		return passport;
	}
	/**
	...
	private void sendImg(Passport passport,PWBRequest request,PassCode passCode){
		...
		if (imagebt != null) {
			...
			passport.setComLogContent("YanChengProcessorImpl_sendImg_Reading image error");
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
			this.reapplySet(passport, passCode.getReapplyCount());
		}
	}
	...
	private byte[] getImageCode(PWBRequest request) {
		...
		try {
			...
			LOG.error("YanCheng getImage Exception message:", e);
		}
		return imageByte;
	}
