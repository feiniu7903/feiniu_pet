 ...
 *
 */
public class OverseasChinatownProcessorImpl implements ApplyCodeProcessor,DestroyCodeProcessor, ResendCodeProcessor, OrderPerformProcessor {
	private static final Log log = LogFactory.getLog(OverseasChinatownProcessorImpl.class);
	...
	public Passport apply(PassCode passCode) {
		log.info("OverseasChinatown apply serialNo: " + passCode.getSerialNo());
		Passport passport = new Passport();
		long startTime=0L;
		String secretKey = WebServiceConstant.getProperties("overseasChinatown.secretKey");
		...
		try {
			...
			String confirmOrderUrl = WebServiceConstant.getProperties("overseasChinatown.url")+"confirmOrder/";
			...
			log.info("OverseasChinatown apply serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			if (confirmOrderResult.startsWith(HttpsUtil.HTTP_ERROR_PREFIX)) {
				...
				if(SUCCESS_CODE.equals(confirmOrderRspCode)){
					...
					log.info("OverseasChinatown apply rspBodyXml"+rspBodyXml);
					...
					passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
					this.reapplySet(passport, passCode.getReapplyCount());
					log.info("OverseasChinatown apply fail message: " + confirmOrderRspCode + " " + rspDesc);
				}
			}
		} catch (Exception e) {
			log.error("OverseasChinatown apply serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			passport.setComLogContent(e.toString());
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
			this.reapplySet(passport, passCode.getReapplyCount());
			log.error("OverseasChinatown applyCode Exception:",e);
		}
		return passport;
	}
	...
	public Passport destroy(PassCode passCode) {
		log.info("OverseasChinatown destroy serialNo: " + passCode.getSerialNo());
		Passport passport = new Passport();
		long startTime=0L;
		passport.setSerialno(passCode.getSerialNo());
		...
		try {
			...
			String url = WebServiceConstant.getProperties("overseasChinatown.url")+"orderRefund/";
			...
			log.info("OverseasChinatown destroy serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			if (orderRefundResult.startsWith(HttpsUtil.HTTP_ERROR_PREFIX)) {
				...
				if(SUCCESS_CODE.equals(orderRefundRspCode)){
					...
					log.info("OverseasChinatown destroy orderRefundRspBodyXml: " + orderRefundRspBodyXml);
			...
			log.error("OverseasChinatown destroy serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			passport.setComLogContent(e.getMessage());
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.DESTROY.name());
			passport.setStatus(PassportConstant.PASSCODE_STATUS.FAILED.name());
			log.error("OverseasChinatown destroy error message", e);
		}
		return passport;
	...
	public Passport resend(PassCode passCode) {
		log.info("OverseasChinatown resend serialNo: " + passCode.getSerialNo());
		String secretKey = WebServiceConstant.getProperties("overseasChinatown.secretKey");
		Passport passport = new Passport();
		...
		try {
			...
			String url = WebServiceConstant.getProperties("overseasChinatown.url")+"resendVoucher/";
			log.info("OverseasChinatown resend reqBodyXml: " + reqBodyXml);
			log.info("OverseasChinatown resend reSendSMSReqUrl: " + url);
			...
			if (resXml.startsWith(HttpsUtil.HTTP_ERROR_PREFIX)) {
				...
				if (SUCCESS_CODE.equals(rspCode)) {
					...
					log.info("OverseasChinatown resend reSendSMSResBody: " + rspBody);
					String rspBodyXml = "<Body>"+DesSecret.decrypt(Base64.decoder(rspBody, "UTF-8"), secretKey)+"</Body>"; 
					String result = TemplateUtils.getElementValue(rspBodyXml, "//Body/Voucher/VoucherResendSuc");
					log.info("OverseasChinatown resend rspBodyXml"+rspBodyXml);
					if("1".equals(result)){
						passport.setStatus(PassportConstant.PASSCODE_STATUS.SUCCESS.name());
					}else{
						passport.setErrorNO(PassportConstant.PASSCODE_ERROR.RESEND.name());
	...
	public Passport perform(PassCode passCode) {
		...
		log.info("OverseasChinatown perform serialNo: " + serialNo);
		String url = WebServiceConstant.getProperties("overseasChinatown.url")+"queryOrderList/";
		String secretKey = WebServiceConstant.getProperties("overseasChinatown.secretKey");
		...
		try {
			...
			String bodyCiphertext = Base64.encoder(DesSecret.encrypt(reqBodyXml, secretKey),"UTF-8");;
			String reqXml = getRequestXml(reqBodyXml, bodyCiphertext);
			log.info("OverseasChinatown perform reqBodyXml: " + reqBodyXml);
			...
			log.info("OverseasChinatown performResponseXml: " + resXml);
			if (resXml.startsWith(HttpsUtil.HTTP_ERROR_PREFIX)) {
				log.info("供应商返回异常：" + resXml.substring(HttpsUtil.HTTP_ERROR_PREFIX.length()));
			} else {
				String rspCode = TemplateUtils.getElementValue(resXml, "//Trade/Head/StatusCode");
				if (SUCCESS_CODE.equals(rspCode)) {
					String rspBody = TemplateUtils.getElementValue(resXml, "//Trade/Body");
					log.info("OverseasChinatown perform rspBody: " + rspBody);
					String rspBodyXml = "<Body>"+DesSecret.decrypt(Base64.decoder(rspBody, "UTF-8"), secretKey)+"</Body>"; 
					...
					if ("2".equals(orderStatus)) {// 已使用
						...
						passport.setDeviceId("OverseasChinatown");
					}
				} else {
					String rspDesc = TemplateUtils.getElementValue(resXml, "//Trade/Body/Message");
					log.info("OverseasChinatown perform fail message: " + rspCode + " " + rspDesc);
				}
			}
		} catch (Exception e) {
			log.error("OverseasChinatown perform error message", e);
		}
		return passport;
	}
	...
	private void fillHeadBean(RequestBean head , String bodyContent) throws NoSuchAlgorithmException{
		...
		String distributorId = WebServiceConstant.getProperties("overseaschinatown.distributorId");
