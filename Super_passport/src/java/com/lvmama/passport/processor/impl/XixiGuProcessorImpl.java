 ...
 * 嬉戏谷服务商实现-对接智友宝
 */
public class XixiGuProcessorImpl implements ApplyCodeProcessor, DestroyCodeProcessor {
	private OrderService orderServiceProxy = (OrderService) SpringBeanProxy.getBean("orderServiceProxy");
	private static final Log LOG = LogFactory.getLog(XixiGuProcessorImpl.class);
	private static final String SUCCESS = "0";
	@Override
	public Passport apply(PassCode passCode) {
		LOG.info("XixiGu Apply Code: " + passCode.getSerialNo());
		...
		if (extId == null || "".equals(extId)) {
			...
			try {
				...
				LOG.info("XixiGu Apply serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				LOG.info("XixiGu xmlResponse:" + xmlResponse);
				...
				if (code.equals(SUCCESS)) {
					...
					LOG.info("XixiGu Apply Error message:" + description);
				}
			} catch (Exception e) {
				LOG.error("XixiGu Apply serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
				passport.setComLogContent(e.getMessage());
				this.reapplySet(passport, passCode.getReapplyCount());
				LOG.error("XixiGu Apply Exception message:", e);
			}
		}else{
			sendImg(passport,request,passCode);
		}
	...
	public Passport destroy(PassCode passCode) {
		LOG.info("XixiGu Destroy Code: " + passCode.getSerialNo());
		Passport passport = new Passport();
		...
		try {
			...
			LOG.info("XixiGu destroy serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			...
			if (response.getCode().equals(SUCCESS)) {
				...
				LOG.info("XixiGu Destroy Success message:" + description);
			} else {
				...
				LOG.info("XixiGu Destroy Error message:" + description);
			}
		} catch (Exception e) {
			LOG.error("XixiGu destroy serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			passport.setStatus(PassportConstant.PASSCODE_STATUS.FAILED.name());
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.DESTROY.name());
			passport.setComLogContent(e.getMessage());
			LOG.error("XixiGu Destroy Exception:", e);
		}
		return passport;
	}
	/**
	...
	private void sendImg(Passport passport,PWBRequest request,PassCode passCode){
		...
		if (imagebt != null) {
			...
			passport.setComLogContent("XixiGuProcessorImpl_sendImg_Reading image error");
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
			this.reapplySet(passport, passCode.getReapplyCount());
		}
	}
	...
	private byte[] getImageCode(PWBRequest request) {
		...
		try {
			...
			LOG.error("XixiGu getImage Exception message:", e);
		}
		return imageByte;
	}
