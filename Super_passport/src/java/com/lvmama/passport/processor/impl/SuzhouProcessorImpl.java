 ...
 *
 */
public class SuzhouProcessorImpl implements ApplyCodeProcessor, DestroyCodeProcessor {
	private OrderService orderServiceProxy = (OrderService) SpringBeanProxy.getBean("orderServiceProxy");
	private static final Log LOG = LogFactory.getLog(SuzhouProcessorImpl.class);
	private static final String SUCCESS = "0";
	@Override
	public Passport apply(PassCode passCode) {
		long startTime=0L;
		LOG.info("suzhouleyuan Apply Code: " + passCode.getSerialNo());
		...
		if (extId == null || "".equals(extId)) {
			try {
				...
				LOG.info("suzhouleyuan Apply serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				LOG.info("suzhouleyuan Apply xmlResponse:" + xmlResponse);
				...
				if (code.equals(SUCCESS)) {
					...
					LOG.info("suzhouleyuan Apply Error message:" + description);

				}
			} catch (Exception e) {
				LOG.error("suzhouleyuan Apply serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
				passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
				passport.setComLogContent(e.getMessage());
				this.reapplySet(passport, passCode.getReapplyCount());
				LOG.error("suzhouleyuan Apply Error message:", e);
	...
	public Passport destroy(PassCode passCode) {
		long startTime=0L;
		LOG.info("suzhouleyuan Destroy Code: " + passCode.getSerialNo());
		Passport passport = new Passport();
		passport.setSerialno(passCode.getSerialNo());
		...
		try {
			...
			LOG.info("suzhouleyuan destroy serialNo :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			LOG.info("suzhouleyuan destroy xmlResponse:"+responseStr);
			...
			if (response.getCode().equals(SUCCESS)) {
				...
				LOG.info("suzhouleyuan Destroy Success message:" + description);
			} else {
				...
				LOG.info("suzhouleyuan Destroy Error message:" + description);
			}
		} catch (Exception e) {
			LOG.error("suzhouleyuan destroy serialNo Error :" +passCode.getSerialNo() +" UseTime:"+ (System.currentTimeMillis() - startTime)/1000);
			passport.setStatus(PassportConstant.PASSCODE_STATUS.FAILED.name());
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.DESTROY.name());
			passport.setComLogContent(e.getMessage());
			LOG.error("suzhouleyuan Destroy Exception:", e);
	...
	private void sendImg(Passport passport,PWBRequest request,PassCode passCode){
		...
		if (imagebt != null) {
			...
			passport.setComLogContent("suzhouleyuan ProcessorImpl_sendImg_Reading image error");
			passport.setErrorNO(PassportConstant.PASSCODE_ERROR.APPLY.name());
			this.reapplySet(passport, passCode.getReapplyCount());
		}
	}
