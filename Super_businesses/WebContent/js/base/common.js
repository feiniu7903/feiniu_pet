$(function() {
	...
	_Utils.showLog = function(type, id){
		var url = "/pet_back/fin/log/search.do?type="+type+"&id="+id;
...
(function( $ ) {
	...
	$.fn.grid = function( option ,pageBarOption ){
		...
		//不允许改变的配置
		var finalConfig = {
				prmNames:{page:"currentPage",rows:"pageSize",sort: "orderBy",order: "order"},
				jsonReader : {
			        root:"items", 
			        page: "currentPage", 
			        total: "totalPageNum", 
			        records: "totalResultSize", 
			        repeatitems: false, 
			        id: "0" 
			    },  
			    datatype : "json",
	...
	$.record_dialog_callback=function(data,div_id){
    	var dialog = art.dialog.get("record_dialog_callback");
    	if(typeof dialog !="undefined"){
    		dialog.close();
    	}
    	art.dialog({
    		id:"record_dialog_callback",
			title: "流水记录",
		    content: data,
		    lock:true
		});
    	if(typeof div_id =="undefined"){
    		div_id = "paginationDiv";
    	}
		var paginationHtml = $("#"+div_id).html();
		$("#"+div_id).html("");
		$("#"+div_id).html(paginationHtml);
		$("#paginationForm").ajaxForm({
			type:"post",
			success:function(data){
				$.record_dialog_callback(data,div_id);
			}
		});
    },
