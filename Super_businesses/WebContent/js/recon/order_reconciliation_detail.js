function changeStatusFn(id,status) { //单击删除链接的操作
		...
		art.dialog({
		    ...
		    ok: function () {
		    	...
		    	$.post(
						"fin_recon_statement!changeStatus.do",
						params,
						function(data){
							if(data != 'SUCCESS'){
								art.dialog({
								...
								//更新汇总信息
								$.post(
									"fin_recon_statement!refreshSummary.do",	
									{'finReconStatementModel.reconStatementId':id}
								);		
							}else{
								$.msg('失败');
	...
	function searchFinReconStatementItemHandler(){
		...
		$("#finReconStatement_item_table").grid({
			...
			colModel : [ 
			             ...
			             {name:'orderId', index:'ORDER_ID',sortable:true, width:100},
			             {name:'orderAmount',sortable:false, width:100,formatter:currencyFmatter,align:'right'},
			             {name:'mainProdId', sortable:false, width:80},
			             {name:'allProdName', sortable:false, width:120},
			             {name:'paymentAmount', sortable:false, width:100,formatter:currencyFmatter,align:'right'},
			             {name:'orgName', sortable:false, width:120},
	...
	function exportXlsFileFn(){
		...
		var reconType=$("#reconType option:selected").val();
		var url="reconStatus="+reconStatus;
		url+="&reconDateFrom="+reconDateFrom;
		url+="&reconDateTo="+reconDateTo;
		url+="&gwName="+gwName;
		url+="&orderId="+orderId;
		url+="&reconType="+reconType;
		url+="&reconSource=1";
		url="fin_recon_statement!xlsFileExport.do?"+url;
