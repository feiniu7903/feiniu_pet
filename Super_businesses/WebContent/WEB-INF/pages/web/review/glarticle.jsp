...
<body>
	...
	<div class="iframe-content">
		  <div class="p_box">
            <form action="${ basePath}/glarticle/query.do" method="post" id="form1">
			...
			<table class="p_table ">
				...
				<s:if test="pagination.allItems!=null ">
					...
					<s:iterator value="pagination.allItems" var="var" status="st">
						<tr>
							<td><a target="_blank" href="${url}">${articleid}</a></td>
							<td>${title}<br />${ description}<br />${content}</td>
						</tr>
						<tr>
							<td><a class="showLogDialog" param="{'parentId':${articleid},'parentType':'glarticle'}"	href="#log">操作日志</a></td>
							<td><input type="hidden" id="id<s:property value='#st.count'/>" value="${articleid}" /> 
				...
				<tr>
					<td></td>
					<td><input class="btn btn-small w5" type="button" value="提交" onclick="javascript:return ajaxSumitPage('${basePath}//glarticle/glarticleupdate.do',${statusId});">
				</tr>
			</table>
		</div>
	</div>
