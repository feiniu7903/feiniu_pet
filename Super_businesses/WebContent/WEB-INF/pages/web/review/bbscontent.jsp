...
<script type="text/javascript">
	...
	<div class="iframe-content">
        <div class="p_box">
            <form action="${ basePath}/bbsContent/query.do" method="post" id="form1">
			...
			<table class="p_table ">
				...
				<s:if test="pagination.allItems!=null ">
					...
					<s:iterator value="pagination.allItems" var="var" status="st">
						<tr>
							<td><a target="_blank" href="${url}">${pid}</a></td>
							<td>${subject}<br />${message}
							</td>
						</tr>
						<tr>
							<td><a class="showLogDialog"
								param="{'parentId':${pid},'parentType':'bbsContent'}"
								href="#log">操作日志</a></td>
							<td><input type="hidden"
								id="id<s:property value='#st.count'/>" value="${pid}" /> <input
								name="reviewstatus<s:property value='#st.count'/>" type="radio"
								value="1" <s:if test='statusId==1||statusId==4||statusId==5' > checked="checked"</s:if> /> 白色 <input
								name="reviewstatus<s:property value='#st.count'/>" type="radio"
								value="2" <s:if test='statusId==2' > checked="checked"</s:if> /> 黑色 &nbsp; <input
								name="reviewstatus<s:property value='#st.count'/>" type="radio"
								value="3"  <s:if test='statusId==3' > checked="checked"</s:if> /> 灰色 
				...
				<tr>
					...
					<td><input class="btn btn-small w5" type="button" value="提交"
						onclick="javascript:return ajaxSumitPage('${basePath}//bbsContent/update.do',${statusId});">
				</tr>
			</table>
		</div>
	</div>
