								...
								<div data-sellbale="true" class="plug_calendar_d_box month_1">
									<table>
										...
										<tr>
												...
												<s:iterator value="#cal" var="superPrice" 	status="substatus">
													<s:if test="#superPrice.specDateCh==#timePrice.specDateCh">
														...
														<s:if test="null!=#superPrice.marketPrice || null!=#timePrice.marketPrice">
															...
															<s:if test="null!=#superPrice.cancelStrategy && null!=#timePrice.cancelStrategy">
															...
															<s:elseif test="null==#superPrice.cancelStrategy && null!=#timePrice.cancelStrategy">
																<input type="hidden" showold="old" columnname="cancelStrategy" 
																value="<s:if test='"FORBID"==#timePrice.cancelStrategy'>不退不改</s:if><s:else>人工确认</s:else>"/>
															</s:elseif>
														</s:if>
											...
											<!-- <td align="right" width="50"  style="text-align:right;" timepriceid="<s:property value="#timePrice.timePriceId" />" tddatevalue="<s:date name="#timePrice.specDate" format="yyyy-MM-dd"/>">
												<s:if test="null!=#timePrice.timePriceId">
													<font class="menshijia" columnname="marketPrice" shownew="new"><s:if test="null!=#timePrice.marketPrice"><s:property value="#timePrice.marketPrice/100" /></s:if></font><br/>
													<font class="shoujia" columnname="price" shownew="new"><s:if test="null!=#timePrice.price"><s:property value="#timePrice.price/100" /></s:if></font><br/>
													<font class="jiesuanjia" columnname="settlementPrice" shownew="new"><s:if test="null!=#timePrice.settlementPrice"><s:property value="#timePrice.settlementPrice/100" /></s:if></font><br/>
													<font class="kucun" columnname="dayStock" shownew="new"><s:if test="null==#timePrice.dayStock"></s:if><s:if test="'UNLIMITED_STOCK'==#timePrice.stockType">不限</s:if><s:else><s:property value="#timePrice.stockTypeShort"/><s:property value="#timePrice.dayStock" /></s:else></font><br/>
													<font class="ziyuan" columnname="resourceConfirm" shownew="new"><s:if test="null!=#timePrice.resourceConfirm"><s:if test='"true"==#timePrice.resourceConfirm'>是</s:if><s:else>否</s:else></s:if><s:else>　</s:else></font><br/>
													<font class="ziyuan" columnname="overSale" shownew="new"><s:if test="null!=#timePrice.overSale"><s:if test='"true"==#timePrice.overSale'>是</s:if><s:else>否</s:else></s:if><s:else>　</s:else></font><br/>
													<s:if test="null!=#timePrice.forbiddenSell"><input type="hidden" shownew="new" columnname="forbiddenSell" value="<s:if test='"true"==#timePrice.forbiddenSell'>关班/满房</s:if><s:else>可订</s:else>"/></s:if>
													<s:if test="null!=#timePrice.stockType"><input type="hidden" shownew="new" columnname="stockType" value="<s:property value="#timePrice.stockTypeCh"/>"/></s:if>
													<s:if test="null!=#timePrice.aheadHour"><input type="hidden" shownew="new" columnname="aheadHour" value="<s:property value="#timePrice.aheadHour"/>"/></s:if>
													<s:if test="null!=#timePrice.cancelStrategy"><input type="hidden" shownew="new" columnname="cancelStrategy" value="<s:if test='"FORBID"==#timePrice.cancelStrategy'>不退不改</s:if><s:else>人工确认</s:else>"/></s:if>
													<s:if test="#timePrice.haveShareStock"><input type="hidden" shownew="new" columnname="shareStockNum" value="<s:property value="#timePrice.shareStockNum"/>"/></s:if>
												</s:if>
											</td> -->
										</tr>
