...
<!--订单详情-->
<form action="${contextPath }/ebooking/task/cancel.do" id="confirm" method="post">
...
<div class="dingdan_cl" id="print_content">
    ...
    <ul class="dingdan_cl_list">
       ...
       <li>
        ...
        <li>
        	<span>酒店确认号：</span><p>${ebkTask.ebkCertificate.supplierNo }</p>
        </li> 
        <li>
        	<span>取消提示：</span>
        	<s:if test="ebkTask.orderCreateTime">
            	<p class="red">订单已通过电话或其他方式征得你方同意接受取消，请确认此单取消。如有疑问，可在备注框中说明。</p>
        	</s:if>
        	<s:else>
	        	<p>用户在订单的最晚取消时间之前申请取消订单，请确认此单取消。如有疑问，可在备注框中说明。</p>
        	</s:else>
        </li>        
        <li>
        	<span>备注：</span><p><textarea class="beizhu_in" id="memo" name="memo" onFocus="if(value=='您可在此输入对取消订单的疑问，若无疑问，可不填'){value=''}" onBlur="if (value ==''){value='您可在此输入对取消订单的疑问，若无疑问，可不填'}">您可在此输入对取消订单的疑问，若无疑问，可不填</textarea></p>
        </li>
        <li>
        	<a href="javascript:void(0)" class="submit_but but_width124" onclick="checkAndSubmit(this);">确认取消</a>
        </li>
    </ul>
</div>
</form>
...
function checkAndSubmit(ele) {
	if($("#memo").val() == "您可在此输入对取消订单的疑问，若无疑问，可不填") {
		$("#memo").val("");
	}
	var options = {
