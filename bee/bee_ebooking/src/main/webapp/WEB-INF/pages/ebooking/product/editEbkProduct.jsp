...
<link rel="stylesheet" href="${contextPath }/css/base/jquery.jsonSuggest.css"/>
...
<script type="text/javascript" src="${contextPath }/js/base/jquery.jsonSuggest.min.js"></script>
<style type="text/css">
...
<body id="body_cpgl">
					...
					<li style="display: block;">
						<table class="jibenxinxi">
							...
							<tr>
								...
								<td><input type="text" name="ebkProdProduct.recommend" maxlength="15" value="${ebkProdProduct.recommend}"  messagetitle="一句话推荐"></td>
								<td><b>(限15个字以内,不得出现公司名称)</b></td>
							</tr>
							<tr>
									...
									<div class="jia_box" id="placeAddBox">
										<s:if test="null!=ebkProdProduct.ebkProdPlaces && ebkProdProduct.ebkProdPlaces.size()>0">
											<s:iterator value="ebkProdProduct.ebkProdPlaces" var="place" status="index">
												<span id='place_box_${place.placeId }'>${place.placeName }<samp class="colsecurrentspan"><font color="red">X</font></samp><input type="hidden" name="ebkProdProduct.ebkProdPlaces[${index.index }].placeName" value="${place.placeName }"><input type="hidden" name="ebkProdProduct.ebkProdPlaces[${index.index }].placeId" value="${place.placeId }"></span>
											</s:iterator>
										</s:if>
									</div>
								</td>
				...
				onSelect:function(item){
					...
					if(item.text!='' && item.text!='undefined' && item.text!=undefined){
						...
						if($("#place_box_"+item.id).val()==undefined){
							$("#placeAddBox").append("<span id='place_box_"+item.id+"'>"+item.text+"<samp class='colsecurrentspan'><font color='red'>X</font></samp><input type='hidden' value='"+item.id+"' name='ebkProdProduct.ebkProdPlaces.placeId'/></span> ");
						}else{
	...
	<script type="text/javascript" src="${basePath }js/ebooking/editEbkProdProductInfo.js"/>
	...
	<link rel="stylesheet"
		href="http://pic.lvmama.com/min/index.php?f=/styles/new_v/ob_common/ui-components.css,/styles/v4/modules/tip.css,/styles/v4/modules/dialog.css,/styles/v4/modules/button.css">
</body>
</html>