...
function showDetail(id){
	$("#housePriceDetailWin").load("routeStockApplyDetail.do",{housePriceId:id},
			function(data){
				tan_show1();
			}
	);
...
<dl class="order_nav">
	<dt>线路库存维护</dt>
    <dd><a href="${basePath}ebooking/routeStock/maintainRouteStockStatus.do">线路库存维护</a></dd>
    <dd class="order_nav_dd"><a href="javascript:void(0)">已提交申请</a></dd>
</dl>
<form id="submitedRouteStockApply" action="${basePath}ebooking/routeStock/submitedRouteStockApply.do" method="post">
<ul class="order_all">
    	...
    	<dl>
            ...
            <dd>
            	<ul class="search_ul_t">
                    ...
                    <li class="search_ul_b_but"><span onclick="$('#submitedRouteStockApply').submit();">查询</span></li> 
                </ul>
            </dd>
        </dl>
    	</div>
	...
	<table width="960" border="0" class="table01">
    	...
    	<tr>
      	  <td class="tab_td1"><s:property value="housePriceId" /></td>
   		  <td><s:property value="metaBranchId" /></td>
   		  <td><s:property value="productName" />(<s:property value="branchName" />)</td>
   		   <td><s:if test="stockAddOrMinus!=0"><s:if test="isAddDayStock=='false'">-</s:if></s:if>
   		   		<s:property value="stockAddOrMinus" />
   		   </td>
   		  ...
   		  <td>
   		  	...
   		  	<s:if test="auditStatus.deleteOrCancel">| 
   		  		<a href="${basePath}ebooking/routeStock/cancelRouteStockApply.do?ebkHousePrice.housePriceId=<s:property value='housePriceId' />">
   		  			撤销</a>
   		  	</s:if>
   		  </td>
   		</tr>    
