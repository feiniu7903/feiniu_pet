					...
					<li style="display: block;">
						<table class="jibenxinxi">
							<tr>
								...
								<td width="220"><span tip-content="目的地x晚x日游*xx直飞  xx住宿（可以是修饰词）"
							...
							<tr>
								...
								<td>
									...
									<span tip-content="只要包含有任何一点行程的都是跟团游；<br>机票+酒店行程都是自行安排的属于出境自由行" class="text_ts tip-icon tip-icon-info"></span>
								</td>
							...
							<tr>
								<td align="right"><span class="red_ff4444">*</span>城市名：</td>
								<td>
									<input type="text"  class="searchInput" id="placeName" messagetitle="城市名">
									<input id="placeId"  name="placeId" type="hidden"  messagetitle="城市名">
								</td>
								<td colspan="3"><b>（如未找到相关城市名，可联系产品经理添加）</b></td>
							</tr>
							<tr>
								<td></td>
								<td colspan="4">
							...
							<s:include value="./subPage/visaBaseInfo.jsp"></s:include>
							<tr class="teshuRadioTr">
			            		<td style="width:137px;" align="right"><span class="red_ff4444">*</span>是否多行程:</td>
			            		<td colspan="3">
			            			<s:if test="ebkProdProduct.ebkProdProductId == null">
			            				<s:radio name="ebkProdProduct.isMultiJourney" list="#{'Y':'是','N':'否'}" id="teshuRadio"></s:radio>
			            				<b>（是否多行程——保存成功后，将无法再进行修改，请慎重选择！）</b>
			            			</s:if>
									<s:else>
										<input type="hidden" name="ebkProdProduct.isMultiJourney" value="${ebkProdProduct.isMultiJourney}" />
										${ebkProdProduct.zhMultiJourney }
									</s:else>
								</td>
			            	</tr>
						</table>
...
<script type="text/javascript">
	$(function() {
		...
		/*送签国家*/
		$("#countryName").jsonSuggest({
			url:"${contextPath}/product/queryVisaCountry.do",
			maxResults: 20,
			minCharacters:2,
			onSelect:function(item){
				$(":hidden[name='ebkProdProduct.country']").val(item.id);
			}
		});
	});
</script>
