...
function showPlay(num, flag) {
	...
	if (flag == true) {
		firstTravellerInfoOptions = 'NAME;' + 'CARD_NUMBER;'
				+ $("#firstTravellerInfoOptions").val(),
				travellerInfoOptions = 'NAME;' + 'CARD_NUMBER;'
						+ $("#travellerInfoOptions").val();
	} else {
		firstTravellerInfoOptions = $("#firstTravellerInfoOptions").val(),
				travellerInfoOptions = $("#travellerInfoOptions").val();
	}
	if (num >= 1 && firstTravellerInfoOptions == "[];"
			&& travellerInfoOptions != "") {
		firstTravellerInfoOptions = travellerInfoOptions
	}
	var arr = [];
	arr.push(firstTravellerInfoOptions);

	var playSubmitHtml = "", html = "", htmlInfo = "", hiddenHtml = "", playInfoHtml = "", num = parseInt(
			num, 10);
	...
	if (num == 0) {
		$("div[id='play-edit']").find("input[type='hidden']").remove();
		$("div[id='play-edit']").find("input[type='text']").remove();
		$("div[data-hide=play]").hide();
		$("div[data-hide=play]").eq(0).find("input[value=4]").attr("valid",
				"false");
		return;
	} else {
		$("div[data-hide=play]").eq(0).find("input[value=4]").attr("valid",
	...
	for ( var i = 1; i <= num; i++) {
		// hiddenHtml += '<input type="hidden" value="" name="travellers[' + i
		// + '].receiverId" />';
		var j = i - 1;
		if (arr[i - 1].indexOf("NAME") >= 0) {
			name = '<input type="hidden" value="" autocomplete="off" names="text-play-person" class="input-text" data-field="receiverId" />'
					...
					+ j
					+ '].fullName" autocomplete="off" names="text-play-person" id="play-name'
					+ i
					+ '" placeholder="姓名" class="input-text" data-field="receiverName" maxlength="20">';// +
			// hiddenHtml += '<input type="hidden" value="" name="travellers[' +
			// i
			// + '].fullName" />';
		}
		...
		if (arr[i - 1].indexOf("CARD_NUMBER") > 0) {
			card = '<div class="selectbox selectbox-small">'
					...
					+
					// '<span class="select-arrow"><i class="ui-arrow-bottom
					// dark-ui-arrow-bottom"></i></span>'+
					'<span value="ID_CARD" class="select-value">身份证</span>'
					+ '</p>'
					+
					/*
					 * '<div class="selectbox-drop">'+ '<ul class="select-results">'+ '<li data-value="ID_CARD">身份证</li>'+ '<li data-value="HUZHAO">护照</li>'+ '</ul>'+ '</div>'+
					 */
					'</div>'
					+ '<input type="hidden" value="" autocomplete="off" names="text-play-person" class="input-text" data-field="receiverId" />'
					+ '<input type="hidden" value="ID_CARD" name="travellers['
					+ j
					+ '].idType" autocomplete="off" names="text-play-person" class="input-text" data-field="cardType" />'
					+ '<input type="text" id="play-cardNum'
					+ i
					+ '" name="travellers['
					+ j
					+ '].idNo" names="text-play-person" autocomplete="off" placeholder="证件号码" data-field="cardNum" class="input-text input-middle">';
			// hiddenHtml += '<input type="hidden" value="" name="travellers[' +
			// i
			// + '].idNo" />';
		}
		...
		if (arr[i - 1].indexOf("MOBILE") > 0) {
			mobil = '<input type="hidden" value="" autocomplete="off" names="text-play-person" class="input-text" data-field="receiverId" />'
					+ '<input type="hidden" value="" autocomplete="off" names="text-play-person" class="input-text" data-field="cardType" />'
					+ '<input type="text" class="play-mobile input-text" id="play-mobile'
					+ i
					+ '" name="travellers['
					+ j
					+ '].mobile" names="text-play-person" autocomplete="off" placeholder="手机号" data-field="mobileNumber">';
		}
		html += '<dl class="xdl">' + '<dt><i class="req">*</i> <span>游玩人' + i
				+ '<span>：</span></span></dt>' + '<dd names="play-person-dd">'
				+ name + '<div class="select-group">' + card + mobil + '</div>'
				+ '</dd>' + '</dl>';
		htmlInfo += '<dl class="xdl"><dt><i class="req">*</i>游玩人' + i
				+ '：</dt><dd></dd></dl>';
		name = "";
		card = "";
		mobil = "";
	...
	$("#play-info").html("");
	$("#play-info").append(hiddenHtml).append(htmlInfo).append(playInfoHtml)
			.hide();
...
function bindEventValid(num) {
	...
	for ( var i = 1; i <= num; i++) {
		...
		if ($dl.eq(i - 1).find("input[data-field=receiverName]").length > 0) {
			$("#play-name" + i).liteValid({
				vGroup : "4",
				valid : [ "empty", "reg" ],
				regExp : "userName",
				msg : [ "请输入游玩人姓名", "姓名只能为中文或英文,长度为2-20字符" ]
			});
		}

		if ($dl.eq(i - 1).find("input[data-field=cardNum]").length > 0) {
			$("#play-cardNum" + i).liteValid({
				vGroup : "4",
				valid : [ "empty", "reg" ],
				regExp : "idcard",
