/**
 * 加减数量 <input type="hidden" name="paramName" id="paramId" minAmt="最小起订数"
 * maxAmt="最大订购数" textNum="用户订购数" buyPeople="订购总人数" people="成人数+儿童数"/>
 * 
 * @param paramId
 * @param operator
 * @return
...
function updateRightShow(eventType) {
	...
	$("[mainBranchName]").each(
			function() {
				if ($(this).val() != '0') {
					mainHtml += '<dd><span class="num"><dfn>&yen;'
							+ $(this).attr("sellPrice")
							+ '</dfn>&times;<i class="price">' + $(this).val()
							+ '</i></span><p>' + $(this).attr("mainBranchName")
							+ '</p></dd>';
				}
			});
	...
	$("[minBranchName]").each(
			function() {
				if ($(this).val() != '0') {
					minHtml += '<dd><span class="num"><dfn>&yen;'
							+ $(this).attr("sellPrice")
							+ '</dfn>&times;<i class="price">' + $(this).val()
							+ '</i></span><p>' + $(this).attr("minBranchName")
							+ '</p></dd>';
				}
			});
...
function updatePage(eventType) {
	$("input[name='paramName']").each(
			function() {
				var selfPack = $(
						"form[name=orderForm] input[name='buyInfo.selfPack']")
						.val() == "true";
				var buyPeopleNum = 0;
				if (selfPack) {
					var adult = parseInt($(
							"form[name=orderForm] input[name=buyInfo.adult]")
							.val());
					var child = parseInt($(
							"form[name=orderForm] input[name=buyInfo.child]")
							.val());
					if (adult != NaN) {
						buyPeopleNum += adult;
					}
					...
					for ( var i = 0; i < paramObjs.length; i++) {
						buyPeopleNum += parseInt($(paramObjs[i]).attr(
								"buyPeopleNum"));
					}
				}
				// 更新游玩人
	...
	$("[productAdditional]").each(
			function() {
				if (!this.checked) {
					$("#addition" + $(this).val()).val(0);
				} else {
					$("#addition" + $(this).val()).val(
							$("#input" + $(this).val()).find("li.selected")
									.attr("data-value"));
				}
			});
	...
	// 触发、优惠活动
	var li = $("#couponActivity").find("div.selectbox-drop")
			.find("li.selected");
	if (typeof li.attr("data-value") != "undefined") {
		if (li.attr("data-value") != ""
				&& li.attr("data-value") != 'userCoupon') {
			li.trigger("click");
...
function createOption(paramId, inputId, type) {
	...
	var buyPeopleNum = 0;
	if (selfPack) {
		var adult = parseInt($("form[name=orderForm] input[name=buyInfo.adult]")
				.val());
		var child = parseInt($("form[name=orderForm] input[name=buyInfo.child]")
				.val());
		if (adult != NaN) {
			buyPeopleNum += adult;
		}
	...
	if (branchType == 'FANGCHA') {// 如果是房差时并且是线路产品只有一个人参数时房差的下拉框必须为1
		...
		case 'OPT':// 最小值-人数值序列,该 操作保持以前的做法不变
			...
			for ( var i = min; i <= buyPeopleNum; i++) {
				...
				$opt = $("<li data-value='" + i + "' " + _selected + ">" + i
						+ "</li>");
		...
		case 'ANY':// 最小值与人数值
			...
			if (min < buyPeopleNum) {
				...
				$opt = $("<li data-value='" + min + "' " + min_selected + ">"
						+ min + "</li>");
				select_ul.append($opt);
				$opt = $("<li data-value='" + buyPeopleNum + "' "
						+ buy_selected + ">" + buyPeopleNum + "</li>");
				$opt.attr("class", "selected");
				select_ul.append($opt);
			} else {
				$opt = $("<li data-value='" + min + "' class='selected'>" + min
						+ "</li>");
				select_ul.append($opt);
			}
			selectObj.find("span.select-value").html(current_val);
		...
		case 'ALL':// 人数值
			...
			$opt = $("<li data-value='" + current_val + "' class='selected'>"
					+ current_val + "</li>");
			select_ul.append($opt);
			selectObj.find("span.select-value").html(current_val);
			break;
