...
function confirmUse(obj) {
	...
	if (ckbox.attr("name") == 'prod_coupon_' + ckbox.attr("productId")) {
		$("input[name='prod_coupon_" + ckbox.attr("productId") + "']")
				.each(
						function() {
							if (ckbox != $(this)
									&& ckbox.parent().find("#youhuiContent")
											.css("display") == "none") {
								...
								$(this).parent().find("#amount").attr("hasAdd",
										"false");
								var productNumInput = $("input[name='buyInfo.buyNum.product_"
										+ $(this).attr("productId") + "']");
		...
		$("input[name='ord_coupon']").each(
				function() {
					if (ckbox != $(this)
							&& ckbox.parent().find("#youhuiContent").css(
									"display") == "none") {
						var info = $(this).parent().find("#couponInfo");
						$(this).parent().find("#youhuiContent").hide();
						$(this).parent().find("#couponCodeId").val("");
...
function createContactContent(i, op) {
	...
	if (op != null && op.length > 5) {
		selectHtml = "<select onchange=\"addReceInfo(" + i + ",this)\">" + op
				+ "</select>";
		useOffen = "<input type=\"checkbox\" checked=\"true\" align=\"absmiddle\" name=\"travellerList["
				+ i + "].useOffen\" />保存到常用姓名";
	}
	return "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\" align=\"center\"><tbody><tr><th>其他游玩人"
			+ (i)
			+ "</th><th></th><th align=\"right\"><span class=\"savename\">"
			+ useOffen
			+ "</span></th>"
			+ "</tr> <tr><td width=\"70\"><span>*</span>姓　　名：</td><td width=\"190\"><input type=\"text\" rName=\"rName\" id=\"recrName"
			+ i
			+ "\" name=\"travellerList["
			+ i
			+ "].receiverName\" class=\"text_inp\" /></td>"
			+ "<td>"
			+ selectHtml
			+ "</td></tr>"
			+ "<tr><td><span>*</span>联系电话：</td><td><input type=\"text\" rMobileNumber=\"rMobileNumber\" id=\"mobileNumber"
			+ i
			+ "\" name=\"travellerList["
			+ i
			+ "].mobileNumber\" class=\"text_inp\" />"
			+ "<tr><td><span>*</span>证件类型：</td><td><select id=\"cardType"
			+ i
			+ "\" name=\"travellerList["
			+ i
			+ "].cardType\"><option value=\"\">请选择证件类型</option><option value=\"ID_CARD\" selected=\"selected\">身份证</option><option value=\"HUZHAO\">护照</option><option value=\"CUSTOMER_SERVICE_ADVICE\">客服联系我时提供</option></select></td><td></td></tr>"
			+ "<div id=\"cardNumDivId"
			+ i
			+ "\" style=\"visibility:none\"><tr><td><span>*</span>证件号码：</td><td><input type=\"text\" onKeyDown=\"return checkType('cardType"
			+ i
			+ "',this)\" id=\"cardNum"
			+ i
			+ "\" rTraveller=\"rTraveller\" name=\"travellerList["
			+ i
			+ "].cardNum\" class=\"text_inp\" /></td><td></td></tr></div>"
			+ "<input type=\"hidden\" id=\"receiverId"
			+ i
			+ "\" name=\"travellerList["
			+ i
			+ "].receiverId\" class=\"text_inp\" /></td><td></td></tr></tbody></table>";
}
...
function checkType(id, obj) {
	...
	var childrenBrithdayInfoSpan = "childrenBrithdayInfoSpan"
			+ id.replace(/cardType/g, "");
...
function updateFetchTicketInfo(obj) {
	$("#fetchTicketUserName").val(
			$(obj).find("option:selected").attr("receiverName"));
	$("#fetchTicketUserMobile").val(
			$(obj).find("option:selected").attr("mobile"));
...
function getParamData() {
	var ordNum = $("[ordNum]");
	var param = "productId=" + $("#_mainProductId").val() + "&visitTime="
			+ $("#allVisitDate").val() + "&leaveTime=" + $("#_leaveTime").val()
			+ "&submitOrder=true&";
	for ( var i = 0; i < ordNum.length; i++) {
		var id = ordNum[i].getAttribute("id");
		var obj = $("#" + id);
		param += "buyNum[" + id + "]=" + obj.val() + "&";
	}
	return param;
}
...
function getJsonData1() {
	...
	for ( var i = 0; i < ordNum.length; i++) {
		...
		var value = obj.val();
		if (value && value > 0) {
			param += "'buyNum[" + id + "]':'" + value + "',";
		}
	}
	var param = "{" + param + "productId:'" + $("#_mainProductId").val()
			+ "',visitTime:'" + $("#allVisitDate").val() + "'";
	var endDate = $("#_leaveTime").val();
	if (endDate != "null" && endDate != "") {
		param += ",leaveTime:'" + endDate + "'";
	}
...
function getJsonData() {
	...
	for ( var i = 0; i < ordNum.length; i++) {
		var id = ordNum[i].getAttribute("id");
		var obj = $("#" + id);
		param += "'ordNum." + (ordNum[i].getAttribute("id")) + "':'"
				+ obj.attr("value") + "',";
	}
	var param = "{" + param + "productId:'" + $("#_mainProductId").val()
			+ "',choseDate:'" + $("#allVisitDate").val() + "'";
	var endDate = $("#_leaveTime").val();
	if (endDate != "null" && endDate != "") {
...
function subOrders() {
	// 验证显示模块是否保存
	var submits = [ "", "frast-submit", "order-submit", "take-submit",
			"play-submit", "em-submit" ], len = submits.length, j = 0;
	var result = true;
	for ( var i = 1; i < len; i++) {
		if (i != 2 && i != 5) {
			if (i == 4) {
				var a = $("div[data-hide='play']");
				if (a && a.is(":hidden")) {
					continue;
				}
			}
			var submit = document.getElementById(submits[i]);
			if (submit) {
				submit.click();
				if (!formSubmitValid[i].isValid) {
					j = i;
					result = false;
					break;
				}
	...
	if (!result) {
		$(window).scrollTop(
				$("#" + submits[j]).parents("div[class^=order-content]")
						.offset().top);
		return false;
	}
	var num = 0;
	var limitFlag = false;
	var selfPack = $("form[name=orderForm] input[name='buyInfo.selfPack']")
			.val() == 'true';
	if (!selfPack && $("#allVisitDate").length > 0) {
		var jsonDate = getJsonData1();
		// var paramDate = getParamData();
		$.ajax({
			type : "POST",
			async : false,
			url : "/product/ajaxCheckPrice",
			data : jsonDate,
			dataType : "json",
			success : function(data) {
				var json = data;
	...
	if (num == 0
			&& $("input[sellName='sellName']").not("input[id^='addition']").length > 0) {
...
function checkReceiver() {
	...
	if (!checkMobile(fetchTicketUserMobile)
			|| fetchTicketUserMobile.length != 11
			|| isNaN(fetchTicketUserMobile)) {
...
function error_tip(elt, msg) {
	var _this = $(elt);
	strHtml = '<span class="zhfs_state zhfs_v_error"><i></i><label class="label_tip">'
			+ msg + '</label></span>';
...
$(function() {
	...
	lv_popjs.setAttribute("href",
			"http://pic.lvmama.com/js/ui/lvmamaUI/css/jquery.common.css");
	document.getElementsByTagName("body")[0].appendChild(lv_popjs);
...
function checkVisitorIsExisted() {
	...
	var subProductType = $form.find("input[name='buyInfo.subProductType']")
			.val();
	...
	if (selfPack == "true") {
		...
		if (productType == 'TICKET' || productType == 'HOTEL'
				|| subProductType == 'FREENESS') {
			var travellerInfoOptions = $("#travellerInfoOptions").val();
			if ((travellerInfoOptions != null && travellerInfoOptions != '')
					|| subProductType == 'FREENESS') { // 酒店，并且有游客必填信息的话，需要校验是否已填写游客信息或目的地自由行
				var $obj = $form
						.find("input[name='travellerList[1].receiverName']");
				if ($obj.size() < 1) {
					return true;
				}
			} else {
...
/**
 * 计算两个日期的时间差. date0 = "2008/08/08"; date0 = "2012/07/27";
...
Date.prototype.dateDiff = function(interval, objDate) {
	...
	case "m":
		return (objDate.getMonth() + 1)
				+ ((objDate.getFullYear() - this.getFullYear()) * 12)
				- (this.getMonth() + 1);
		// 計算年差
	case "y":
...
Date.prototype.isLeapYear = function() {
	return ((0 == this.getFullYear() % 4) && ((this.getFullYear() % 100 != 0) || (this
			.getFullYear() % 400 == 0)));
}