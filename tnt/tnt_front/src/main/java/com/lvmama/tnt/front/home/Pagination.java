...
public class Pagination {
	...
	public static String paginationVo(PageConfigVo<?> page,int mode){
		if (page != null && page.getTotalPageNum() > 0){
			Pagination pagination = new Pagination();
			pagination.setPageSize(page.getPageSize());
			pagination.setTotalPage(page.getTotalPageNum());
	...
	public void createTag(int type){
		try {
			...
			case 2:
				str += "<span>"+this.page+"/"+this.totalPage+"</span>";
				if (prevPage < 1) {
					str +=" <a class=\"page-prev\" href=\"#\" title=\"上一页\" rel=\"nofollow\">上一页<span class=\"page-arrow page-arrow-left\"></span></a>";
				} else {
					str +=" <a class=\"page-prev\" href=\"#\" searchvo=\""+this.action.replace("{p}", String.valueOf(prevPage))+"\" searchCondition=\"search\" title=\"上一页\" rel=\"nofollow\">上一页<span class=\"page-arrow page-arrow-left\"></span></a>";
				}
				if (nextPage > this.totalPage) {
					str +=" <a class=\"page-next\" href=\"#\" title=\"下一页\" rel=\"nofollow\">下一页<span class=\"page-arrow page-arrow-right\"></span></a>";
				} else {
					str +=" <a class=\"page-next\" href=\"#\" searchvo=\""+this.action.replace("{p}", String.valueOf(nextPage))+"\" searchCondition=\"search\" title=\"下一页\" rel=\"nofollow\">下一页<span class=\"page-arrow page-arrow-right\"></span></a>";
				}
				break;
			case 3:
				long endPage3 = 0;
				if (prevPage >= 1) {
					str += "<a href=\"#\" searchvo=\""+this.action.replace("{p}", String.valueOf(prevPage))+"\" searchCondition=\"search\" title=\"上一页\" class=\"PrevPage\" rel=\"nofollow\">上一页</a>";
				}
				if (this.page != 1)
				str +="<a href=\"#\" searchvo=\""+this.action.replace("{p}", String.valueOf(1))+"\" searchCondition=\"search\" title=\"到第一页\" class=\"PageLink\"  rel=\"nofollow\">1</a>";

				if (this.page >= 5)
					str +="<span class=\"PageMore\">...</span>";
				if (this.totalPage > this.page + 5) {
					endPage3 = this.page + 5;
				} else {
					endPage3 = this.totalPage;
				}
				for (long i = this.page -5; i <= endPage3; i++) {
					if (i > 0) {
						if (i == this.page) {
							str +="<span class=\"PageSel\">"+i+"</span>";
						} else {
							if (i != 1 && i != this.totalPage) {
								str +="<a href=\"#\" searchvo=\""+this.action.replace("{p}", String.valueOf(i))+"\" searchCondition=\"search\" class=\"PageLink\" title='第 " + i + " 页'  rel=\"nofollow\">" + i
										+ "</a>";
							}
						}
					}
				...
				if (this.page + 3 < this.totalPage)
					str +="<span class=\"PageMore\">...</span>";
				if (this.page != this.totalPage)
					str +="<a  href='#' searchvo=\"" +this.action.replace("{p}", String.valueOf(this.totalPage))+"\" searchCondition=\"search\" class=\"PageLink\" title= '第 " + this.totalPage
							+ " 页'  rel=\"nofollow\">" + this.totalPage + "</a>";

				if (nextPage <= this.totalPage) {
					str += "<a href='#' searchvo=\"" +this.action.replace("{p}", String.valueOf(nextPage))+"\" searchCondition=\"search\" title=\"下一页\" class=\"NextPage PageLink PageLink_page\"  rel=\"nofollow\">下一页</a>";
				}
				str +="</span>";
				break;	
			case 12:
