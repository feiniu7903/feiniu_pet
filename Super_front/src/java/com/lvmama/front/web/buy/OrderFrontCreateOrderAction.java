...
@Results( {
		/*@Result(name = "view", location = "/pay/ticket-${buyInfo.productId}-${orderId}", type = "redirect"),*/
...
public class OrderFrontCreateOrderAction extends OrderCreateOrderAction {
	private static final long serialVersionUID = 7759541215656060198L;
	private static final Log LOG = LogFactory.getLog(OrderFrontCreateOrderAction.class);
	...
	public String execute() throws Exception {
		...
		try {			
			...
			if(MemcachedSeckillUtil.getInstance().get(Constant.BLACK_LIST.BLACK_BY_MOBILE_AND_PRODUCT_SUBMIT_ORDER_UUID+buyInfo.getProductId().toString()+this.getContact().getMobileNumber()) != null){
				LOG.info("=====================并发  validateBlackByMobileAndProduct =================");
				return ERROR;
			}
			//黑名单
			MemcachedSeckillUtil.getInstance().set(Constant.BLACK_LIST.BLACK_BY_MOBILE_AND_PRODUCT_SUBMIT_ORDER_UUID+buyInfo.getProductId().toString()+this.getContact().getMobileNumber(),true);
			boolean flag = ProductBlackValidateUtil.getProductBlackValidateUtil().validateBlackByMobileAndProduct(buyInfo.getProductId(), this.getContact().getMobileNumber());
			if(!flag){
				LOG.info("============validate blackListProd validateBlackByMobileAndProduct ==============="+flag);
				return ERROR;
			}
			//检验是否黑名单
			try{
