...
public class OrderCreateOrderAction extends BaseAction {
	...
	public String createOrder() throws Exception {
		...
		createOrderBuyInfo.setCouponList(favorService.validateCouponListByProductCouponUseFlag(this.getCoupon(), prodProduct));
...
//		}
		...
		if(ActivityUtil.getInstance().checkActivityIsValid(Constant.ACTIVITY_FIVE_YEAR) || !Constant.initCouponEnabled(buyInfo.getProductId())){
...
//		}
		...
		try {
			...
			if(MemcachedSeckillUtil.getInstance().get(Constant.BLACK_LIST.BLACK_BY_MOBILE_AND_PRODUCT_SUBMIT_ORDER_UUID+buyInfo.getProductId().toString()+this.getContact().getMobileNumber()) != null){
				MemcachedSeckillUtil.getInstance().remove(Constant.BLACK_LIST.BLACK_BY_MOBILE_AND_PRODUCT_SUBMIT_ORDER_UUID+buyInfo.getProductId().toString()+this.getContact().getMobileNumber());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
...
//				}
	...
	/**
	 * 验证购买数量是否小于最低购买数,验证是否存在非法数据
	 * @param
	 */
	public boolean validateSubmitDate(){
		try { 
			//判断取票人数据是否合法
			if(this.contact == null || this.contact.getReceiverName() == null || this.contact.getMobileNumber() == null || StringUtils.isEmpty(this.contact.getReceiverName().trim()) || StringUtils.isEmpty(this.contact.getMobileNumber().trim())){
				LOG.error("=====用户:"+getUserId()+"下单时=======取票人Contact为NULL");
				return true;
			}
			BuyInfo buyInfo = this.getOrderInfo();
			List<Item> itemList = buyInfo.getItemList();
			ProdProduct prodProduct = pageService.getProdProductByProductId(this.buyInfo.getProductId());
			...
			if(!prodProduct.IsAperiodic()) {
				//关联的附加商品.
				relateList = this.productServiceProxy.getRelatProduct(this.buyInfo.getProductId(), this.buyInfo.getVisitDate());
				for (int i = 0; i < relateList.size(); i++) {
					relateListMap.put(relateList.get(i).getProdBranchId(),relateList.get(i));
				}
			}
			int buyNum = 0;   //购买总数
			for (Item item : itemList) {
				...
				if(relateListMap.get(item.getProductBranchId()) == null){
					ProdProductBranch branch = pageService.getProdBranchByProdBranchId(item.getProductBranchId());
					Long adultQuantity = branch.getAdultQuantity() == null ? 0L : branch.getAdultQuantity();
					Long childQuantity = branch.getChildQuantity() == null ? 0L : branch.getChildQuantity();
					buyNum += (item.getQuantity()*(adultQuantity+childQuantity));
				}
			}
			for (Item item : itemList) {
				if(relateListMap.get(item.getProductBranchId()) != null){
					ProdProduct product = pageService.getProdProductByProductId(item.getProductId());
					//保险、税金 人数应与购买人数一致
					if(StringUtils.equals(product.getSubProductType(),Constant.SUB_PRODUCT_TYPE.INSURANCE.name()) || StringUtils.equals(product.getSubProductType(),Constant.SUB_PRODUCT_TYPE.TAX.name())){
						if(buyNum != item.getQuantity()){
							LOG.error("=====用户:"+getUserId()+"下单时=======存在错误数据");
							return true;
						}
					}else if(StringUtils.equals(product.getSubProductType(),Constant.SUB_PRODUCT_TYPE.EXPRESS.name())){ //快递 
						if(item.getQuantity() != 1){
							LOG.error("=====用户:"+getUserId()+"下单时=======存在错误数据");
							return true;
						}
					}else{
						ProdProductRelation prodRelation = relateListMap.get(item.getProductBranchId());
						if(null != prodRelation){
							ProdProductBranch branch = prodRelation.getBranch();
							int miniNum = branch.getMinimum() == null ? 0 : branch.getMinimum().intValue();
							int maxNum = branch.getMaximum() == null ? 0 : branch.getMaximum().intValue();
							if(prodRelation.getSaleNumType().equals(Constant.SALE_NUMBER_TYPE.OPT.name())){
								if(item.getQuantity() < miniNum || (maxNum != 0 && item.getQuantity() > maxNum)){
									LOG.error("=====用户:"+getUserId()+"下单时=======存在错误数据");
									return true;
								}
							}else if(prodRelation.getSaleNumType().equals(Constant.SALE_NUMBER_TYPE.ANY.name())){
								if(item.getQuantity() < miniNum || (maxNum != 0 && item.getQuantity() > maxNum)){
									LOG.error("=====用户:"+getUserId()+"下单时=======存在错误数据");
									return true;
								}
							}else if(prodRelation.getSaleNumType().equals(Constant.SALE_NUMBER_TYPE.ALL.name())){
	...
	private UsrReceivers replaceHiddenMobile(UsrReceivers usrReceivers,List<UsrReceivers> receiversList){
		...
		if(StringUtils.isNotEmpty(usrReceivers.getEmail())){ 
			Pattern pattern = Pattern.compile("\\w+([-+.]\\w+)*[\\*]{4,4}@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*"); 
			if(StringUtils.isNotBlank(usrReceivers.getReceiverId())){//选择的常用联系人 
				Matcher matcher = pattern.matcher(usrReceivers.getEmail()); 
				if (matcher.matches()) {//替换掉**** 
					for(UsrReceivers receiver:receiversList){ 
						if(receiver.getReceiverId().equals(usrReceivers.getReceiverId())){ 
							usrReceivers.setEmail(receiver.getEmail()); 
							break; 
						} 
					} 
				} 
