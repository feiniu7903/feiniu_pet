...
<h3><s></s>入住人信息<em>（请准确填写入住人信息，我们将向取票人手机发送订单短信）</em></h3>
<dl class="user-info">
		...
		<br/>
    		...
    		<!-- 进行格式布局显示,每一行显示5列. -->
    			<@s.if test="((#st.index) % 5 == 0)"><tr></@s.if>
    				<li>
    					<input type="checkbox" name="_recerverName" value="${receiverId?if_exists}" card_type="${cardType?if_exists}" card_num="${cardNum?if_exists}" mobile="<@s.property value="@com.lvmama.comm.utils.StringUtil@hiddenMobile(mobileNumber)" />" receiverName="${receiverName?if_exists}" class="inp-checkbox" onclick="updateFetchTicketInfo2(this);"/> 
		...
		<@s.if test='contactInfoOptions.contains("CARD_NUMBER")'>
    		...
    		<dd>
				...
				<span id="childrenBrithdayInfoSpan1" name="childrenBrithdayInfoSpan" style="display:none">
						<li class="add_xu_add">
						   <span>
						   	<label id="brithdayDateLabelId" class="laber_names"><em id="brithdayDateEmId">*</em>出生年月：</label> <input id="brithdayDate1" name="contact.brithday" type="text" value=""  readonly="true"/>
							</span> 
						</li>
					</span>		
...
<@s.if test="mainProdBranch.prodProduct.physical">
	<@s.if test="isLogin()">
