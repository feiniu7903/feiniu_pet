  <#include "/WEB-INF/pages/buy/bankTipsLayer.ftl"/>
  <@s.if test='order.isCanToPay()'>
	 ...
	 var moneyAccountPay="false";
		...
		function beforeSubmitGateway(a, gateway, noCardFlag){
			...
			//var prodName = "${order.mainProduct.zhProductType}"+"-"+"${order.mainProduct.productName}";
			//window.open('${constant.paymentUrl}pay/'+gateway+'.do?objectId=${order.orderId}&amount='+payAmountFen+'&objectType=${objectType}&paymentType=${paymentType}&waitPayment=${waitPayment}&approveTime=${approveTime}&visitTime=${visitTime}&bizType=${bizType}&objectName='+prodName+'&royaltyParameters=${royaltyParameters?if_exists}&signature='+signature+'&noCardFlag='+noCardFlag,'lvmamaPay');
			document.getElementById("submit_front_pay").action = '${constant.paymentUrl}pay/'+gateway+'.do?objectId=${order.orderId}&amount='+payAmountFen+'&objectType=${objectType}&paymentType=${paymentType}&waitPayment=${waitPayment}&approveTime=${approveTime}&visitTime=${visitTime}&bizType=${bizType}&royaltyParameters=${royaltyParameters?if_exists}&signature='+signature+'&noCardFlag='+noCardFlag;
			document.getElementById("submit_front_pay").submit();
		}
	...
	<form name="submit_front_pay" method="post" id="submit_front_pay"
		action="" target=_blank>
		<input type="hidden" name="objectName" value="${order.mainProduct.zhProductType}-${order.mainProduct.productName}">
	</form>	
	
     <@s.if test='order.isPaymentChannelLimit()'> 
        <#include "/WEB-INF/pages/buy/orderPaymentChannelLimit.ftl"/> 
     </@s.if> 
    <@s.elseif test='isHasAlipayLogin()'>
    	<#include "/WEB-INF/pages/buy/orderPaymentAlipay.ftl"/>
    </@s.elseif>
	<@s.else>
	  <#include "/WEB-INF/pages/buy/orderPayInfo.ftl"/>
	</@s.else>
</@s.if>
