...
<div class="wrap">
	<div class="train-head">
        ...
        <i></i>
        <span><#if lineStation??>${lineStation.stationName}</#if>到<#if lineStation2??>${lineStation2.stationName}</#if>火车时刻表</span>
    </div>
    <div class="train-main">
        ...
        <div class="train-content">
            <h2><#if lineStation??>${lineStation.stationName}</#if>到<#if lineStation2??>${lineStation2.stationName}</#if>火车时刻表<span>共有<i>${lineStationStations.size()}</i>列车次</span></h2>
            ...
            <table class="train-station" cellspacing="0" cellpadding="0">
                ...
                <tbody>
                <#if lineStationStations?? && lineStationStations?size gt 0>
                	<#list lineStationStations as bean>
                <tr data-type="${bean.lineName ? substring(0,1)}">
                    <td>
                        <a class="train-num"  target="_blank" href="http://www.lvmama.com/train/checi/${bean.lineName}">${bean.lineName}</a>
                    </td>
                    <td>
                        <span class="<@s.if test='${bean.departureStationId}==${bean.startStationId}'>icon_start</@s.if><@s.else>icon_pass</@s.else>">${bean.startStationName}</span>
                        <br>
                        <span class="<@s.if test='${bean.arrivalStationId}==${bean.endStationId}'>icon_end</@s.if><@s.else>icon_pass</@s.else>">${bean.endStationName}</span>
                    </td>
                    <td>
                    ...
                    <a href="http://www.lvmama.com/search/train/${stationPinyin}.html?date=${defaultDate}&line=${bean.lineName}" hidefocus="false" class="train-ordering"></a>
                    </td>
                </tr>
                </#list>
                </#if>
					...
					<p class="noResultCity">抱歉，您查询的<span><#if lineStation??>${lineStation.stationName}</#if>到<#if lineStation2??>${lineStation2.stationName}</#if></span>暂无车次</p> 
            ...
            <div class="train-tabs">
                <h4>
                    <a href="#" class="train-selected"><#if lineStation??>${lineStation.stationName}</#if>出发火车时刻表</a>
                    <a href="#"><#if lineStation2??>${lineStation2.stationName}</#if>到达火车时刻表</a>
                </h4>
                <dl class="trains-wrap">
                 ...
                 <#if lineStations2?? && lineStations2?size gt 0>
                    <#list lineStations2 as stationBean>
                        <a href="http://www.lvmama.com/train/shikebiao/${stationBean.stationPinyin}">${stationBean.stationName}火车时刻表</a>
                    </#list>
                 </#if>
                    </dd>
