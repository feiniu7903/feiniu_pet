<tr>
	<th><b>*</b><label>我的出发地:</label></th>
	<td>
		省
		<select name="province" id="provinceId" onChange="updateCities(this.value)">
		...
		<p></p>
		<p>我们会根据您的出发地，为您推荐最合适的旅游产品及资讯</p>
	</td>
</tr>
...
if (value =="320000") {document.getElementById("cityId").options.add(new Option("常熟","320002"));}
			for(var i = 0; i < document.getElementById("cityId").options.length;i++) {  
				if (document.getElementById("cityId").options[i].value == '<@s.property value="subscribe.city"/>')                
			    	document.getElementById("cityId").options[i].selected ="true"; 
			}
}

$(function(){
	var province = '<@s.property value="subscribe.province"/>';
	$(document.body).ready(function(){
		$('#provinceId').find("option").each(function(){
