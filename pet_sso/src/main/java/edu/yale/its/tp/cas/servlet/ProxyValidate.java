 ...
 * Authentication Service. Subclassed (versus collapsed into superclass) in
 * order to provided rigid segmentation, at runtime, between caches, even though
 * they share an implementation.
 */
public class ProxyValidate extends ServiceValidate {

	private static final long serialVersionUID = 7735089394731550344L;
	// *********************************************************************
	// Private state
	...
	public void init(ServletConfig config) throws ServletException {
		...
		serviceValidate = app
				.getInitParameter("edu.yale.its.tp.cas.serviceValidate");
		if (serviceValidate == null)
			throw new ServletException(
					"need edu.yale.its.tp.cas.serviceValidate");
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * happens to be implemented by our superclass. Otherwise, we invoke the
		 * superclass directly, which lets modifications we've made in init()
		 * take effect.
		 */
		String ticketString = request.getParameter("ticket");
		if (ticketString != null && ticketString.startsWith("ST"))
			app.getRequestDispatcher(serviceValidate)
	...
	protected void validationSuccess(PrintWriter out, ServiceTicket st,
		...
		out.println("    <cas:proxies>");
		{
			...
			while (proxies.hasNext())
				out.println("      <cas:proxy>" + proxies.next()
						+ "</cas:proxy>");
		}
		out.println("    </cas:proxies>");
		out.println("  </cas:authenticationSuccess>");
