	...
	<head>
		...
		function oneCityOneCard(oneCityOneCardFlag, historyDiv) {
			if (oneCityOneCardFlag == 'true') {
				...
				window.open('<%=basePath%>/ord/refundMent/ordOrderRefundAdd.zul?orderId=${orderDetail.orderId}&isCancelOrder=true&cancelResson=' + cancelResson,
...
function updateContract(orderId){
	...
	<body>
		...
		<div class="orderpoptit">
			...
			<p class="inputbtn">
				<input type="button" name="btnCloseOrder" class="button" value="关闭"
					...
					<input type="hidden" name="isSHHolidayOrder" class="isSHHolidayOrder" value="${orderDetail.isShHolidayOrder }">
			</p>
		</div>
			...
			<table style="font-size: 12px" width="100%" border="0"
				...
				<tr>
					<td width="20%">
						...
						<font color="red">${orderDetail.zhIsAperiodic }</font>
						<s:if test="orderDetail.testOrderFlag == 'true'"><font style="color:red;">(测试单)</font></s:if>
					</td>
					<td width="30%">
					...
					<td width="30%">
						支付等待时间：<span id="waitPaymentSpan"> 
				...
				<tr>
					...
					<td>
						实付金额：
						<span style="color: #007500; font-weight: bold">${orderDetail.actualPayYuan} </span>
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						...
						<span style="color: #f00; font-weight: bold"> 
							...
							<s:else>
								<fmt:formatNumber value="${orderDetail.actualPayYuan - sumRefundmentAmount}" pattern="#.##" />
							</s:else> 
						</span>
					</td>
				</tr>
				<tr>
					<td colspan="2">
						优惠券(优惠活动)优惠金额：${youHuiAmountList }
					</td>
					<td colspan="2">
						优惠券(优惠活动)名称：${youHuiNameList }
					</td>
				</tr>
				<tr>
					<td>
						早订早惠优惠金额：${earlyCouponAmount }
					</td>
					<td colspan="2">
						优惠名称：${earlyCouponName }
					</td>
					<td colspan="1">
				...
				<tr>
					...
					<td>
						结算状态：${settlementStatus }<input id="settlementStatus" type="hidden" value="${orderDetail.settlementStatus}" />
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<s:if test="orderDetail.needContract=='NEED_ECONTRACT'"> 
							合同：
							<s:if test="orderDetail.eContractStatus!='CONFIRM'&&orderDetail.orderStatus=='CANCEL'">
								已作废
							</s:if>
							...
							<s:elseif test="orderDetail.paymentStatus!='PAYED'||orderDetail.approveStatus!='VERIFIED'">
								已签约未生效
							</s:elseif>
				...
				<tr>
					<s:if test="orderDetail.hasSupplierChannelOrder">
						<td>订单供应商渠道：<span style="color:red;font-weight:bold">${orderDetail.zhSupplierChannelOrder}</span></td>
					</s:if>
					<s:if test="orderDetail.travelGroupCode!=null">
						<td>
							签证状态：
					...
					<td colspan="2">
						用户备注：${orderDetail.userMemo }
 						  <s:if test='orderDetail.orderRoute.groupWordStatus=="SENT_NOTICE"||
 						      orderDetail.orderRoute.groupWordStatus=="MODIFY_NOTICE"'>
 						       <input type="button" id="btnGroupAdviceStatus" value="已通知用户出团通知书"
					               onclick="changeGroupAdviceStatus()" style="color:#ccc;" disabled=disabled />
					           <input type="button" id="batchSendNotifyBtn" value="重发用户出团通知书"  onclick="batchSendNotify('${orderDetail.orderId}');"   />
					      </s:if>
					      <s:elseif test='orderDetail.orderRoute.groupWordStatus=="SENT_NO_NOTICE"||
 						             orderDetail.orderRoute.groupWordStatus=="MODIFY_NO_NOTICE"'>
 						       <input type="button" id="btnGroupAdviceStatus" value="已通知用户出团通知书"  onclick="changeGroupAdviceStatus()"   />
 						       <input type="button" id="batchSendNotifyBtn" value="重发用户出团通知书"  onclick="batchSendNotify('${orderDetail.orderId}');"   />
 						  </s:elseif>      
					</td>
					<td>
				...
				<tr>
					<td>
						产品经理：${managerUser.realName }
					</td>
					<td>
						联系电话：${managerUser.mobile }
					</td>
				...
				<!-- 延长支付等待时间控件 -->
					<td colspan="4">						
						<s:if test="showDelayWaitPaymentFlag">							
							延长支付等待时间：
				...
				<s:if test="groupComAffixList!=null&&groupComAffixList.size()>0">
					...
					<table width="60%">
						<s:iterator value="groupComAffixList" id="affix">
								<tr>
									<td>
										<span title="${affix.memo}">${affix.name}</span>
									</td>
									<td width="80">
		...
		<div class="popbox">
			...
			<p class="paytime">
				游玩时间：
				<s:if test="orderDetail.IsAperiodic()">
					<s:if test="orderDetail.visitTime == null">
					未确认
					</s:if>
					<s:else>
			...
			<s:if test="orderDetail.orderType=='TRAIN'">
				退款说明： <br/>
				退票后，20个工作日，给用户退款。 <br/>
			...
			<s:if test="orderDetail.hasSelfPack()">
					...
					<table style="font-size: 12px" cellspacing="1" cellpadding="4"
						...
						productId="${orderDetail.mainProduct.productId}" prodBranchId="${orderDetail.mainProduct.prodBranchId}">${orderDetail.mainProduct.productName}</a>
							...
							<td>
								${orderDetail.mainProduct.zhProductType }
							</td>
							<td>
								${orderDetail.mainProduct.zhAdditional }
							</td>
							<td>
								${zhVisitTime}
							</td>
						...
						<s:iterator id="prod" value="orderDetail.ordOrderItemProds">
							<s:if test="#prod.productType=='HOTEL'">
								<tr bgcolor="#ffffff">
									...
									<td>
										<a class="showImportantTips" href="javascript:void(0)"
						productId="${prod.productId}" prodBranchId="${prod.prodBranchId}">${prod.productName
											}</a>
									</td>
									<td>
										${prod.priceYuan }
					...
					<s:iterator value="orderDetail.allOrdOrderItemMetas" var="meta">
						<tr bgcolor="#ffffff">
							...
							<td><s:if test="taken='true'">是</s:if>
								<s:else>否</s:else></td>
							<s:if test="orderDetail.IsAperiodic() && aperiodicStatusMap != null">
								<td><s:property value="%{aperiodicStatusMap.get(orderItemMetaId)}"/></td>
							</s:if>
						</tr>
					</s:iterator>
				</tbody>
		...
		<s:if test="orderDetail.orderTrafficList!=null&&!orderDetail.orderTrafficList.isEmpty()">
		<div>
			<table style="font-size: 12px" cellspacing="1" cellpadding="4"
				border="0" bgcolor="#666666" width="100%" class="newfont06">
				<tbody>
					<tr class="CTitle">
						<td height="20" align="left" style="font-size: 14px;"
							colspan="9">交通产品出票信息</td>
					</tr>
					<s:iterator value="orderDetail.orderTrafficList" var="traffic">
						<tr bgcolor="#ffffff">
							<td>订单子号：${traffic.orderItemMetaId}</td>
							<td>出票状态：${traffic.zhStatus}</td>
							<td><s:if test="traffic.status=='fail'">失败原因：${traffic.failReason}</s:if></td>
						</tr>
						<s:iterator value="#traffic.orderTrafficTicketInfoList" var="ticket">
						<tr bgcolor="#ffffff">
							<td>游客：<s:if test="person">${ticket.person.name}</s:if></td>
							<td>座位：${ticket.seatNo}</td>
							<td>价格：${ticket.priceYuan}</td>
						</tr>
						</s:iterator>
					</s:iterator>
				</tbody>
	...
	contactReceiverId = '${orderDetail.contact.receiverId }';
							...
							<s:iterator id="person" value="orderDetail.personList">
								<s:if test="#person.personType=='TRAVELLER'">
									<tr bgcolor="#ffffff" align="center">
										...
										<td>
											<a href="/pet_back/visa/approval/showVitMaterial.do?searchOrderId=${orderDetail.orderId}&searchPersonId=${person.personId }" target="_visa">${person.name }</a>
										</td>
										<td>${person.pinyin }</td>
										<td>
											${person.mobile }
									...
									<td colspan="14">
										...
										<input name="personOrderId" type="hidden" id="personOrderId"
											value="${orderDetail.orderId }" />
												 <s:if test="!orderDetail.isJinjiangOrder">
							...
							<s:iterator value="orderDetail.orderTrafficList" var="traffic">
								<s:iterator value="#traffic.orderTrafficTicketInfoList" var="ticket">
									<tr bgcolor="#ffffff" align="center">
										...
										<td><s:date name="orderDetail.visitTime" format="yyyy-MM-dd"/></td>
				...
				<s:if test='"NEED_ECONTRACT" == orderDetail.needContract &&  "CANCEL"!=orderDetail.orderStatus'>
				<table style="font-size: 12px" cellspacing="1" cellpadding="4"
					...
					<tbody>
						...
						<tr>
							<td><input type="button" value="更新发送合同"	title="若修改了订单信息，请点击此按钮更新合同后发送" class="right-button08"
				...
				<div class="ordersum" style="margin-left: 10px;" id="choseCoupon"
					href="<%=basePath%>shoppingCard/loadChoseCoupon.do">
				</div>
			<!-- 已使用的优惠券(活动) -->
			...
			<p class="submitbtn2">
				...
				<s:else>
					<s:if test="aperiodicCanOperate">
						<s:if test="orderDetail.isCancelAble()">
							<s:if test="orderDetail.paymentTarget == 'TOSUPPLIER' && orderDetail.performStatus=='UNPERFORMED' && !orderDetail.hasSettlement">
								<input type="button" name="btnCancelOrder"
									onclick="chkCancelOrder();" value="废 单" class="right-button08">
							</s:if>
							
							<s:elseif test="orderDetail.isJinjiangOrder && orderDetail.paymentStatus == 'PAYED'">
								<input type="button" name="btnCancelOrder1"
									onclick="jinJiangCancelOrder();" value="废 单" class="right-button08">
							</s:elseif>
							
							<s:elseif test="orderDetail.paymentTarget == 'TOLVMAMA' && orderDetail.approveStatus=='VERIFIED' && orderDetail.paymentStatus == 'UNPAY'">
								<input type="button" name="btnCancelOrder1"
									onclick="chkCancelOrder();" value="废 单" class="right-button08">
							</s:elseif>
						...
						<s:else>
							<span style="color: red">
								<s:if test="orderDetail.isForbid()">
									该订单不退不改
								</s:if>
								<s:else>
									该订单过了最晚废单时间
				...
				<script>
						...
						function chkOneCityOneCard(){
							...
							if('true'==$.trim($('.isSHHolidayOrder').val())){
								doCancelOrder('historyDiv');
								return true;
							}
							if(settlementStatus.value != 'UNSETTLEMENTED'){
                        ...
                        function cancelAndCreateNewOrder() {
                            if (confirm('你确定要废单重下吗？')) {
                                if ($("#cancelReorderReason").val() == '') {
                                    alert("请选择订单重下原因");
                                    return;
                                }
                                $("#reorderForm").submit();
				...
				<input type="button" name="btnCloseOrdHis" class="button" 
			...
			<s:if test="orderDetail.resourceConfirmStatus=='LACK'">
				...
				<table style="font-size: 12px" cellspacing="1" cellpadding="4"
					...
					<tr bgcolor="#ffffff" align="center">
						<td height="25">
							<s:if test="orderDetail.resourceLackReason=='NO_RESOURCE'">
								没有资源
							</s:if>
							<s:elseif test="orderDetail.resourceLackReason=='PRICE_CHANGE'">
								价格更改
							</s:elseif> 
							<s:elseif test="orderDetail.resourceLackReason=='UNABLE_MEET_REQUIREMENTS'">
								无法满足游客要求
							</s:elseif> 
							<s:else>
								其他
			...
			<p class="submitbtn2">
				<s:if test="aperiodicCanOperate">
					<!-- 废单重下按钮 -->
					<s:if test="!orderDetail.isShHolidayOrder && !orderDetail.isJinjiangOrder && orderDetail.isCancelAble() && orderDetail.orderStatus=='NORMAL' && orderDetail.settlementStatus == 'UNSETTLEMENTED' && !hasValidRefundment && orderDetail.performStatus =='UNPERFORMED' && !orderDetail.hasDistribution()">
						<form id="reorderForm" method="post"	action="/super_back/ord/cancelAndCreateNewOrder.do">
							<input type="hidden" name="orderId" value="${orderId}" />
                    <s:select list="@com.lvmama.comm.vo.Constant$CANCEL_REORDER_REASON@values()"
                              listKey="code" listValue="cnName"
                              headerKey="" headerValue="请选择"
                              name="cancelReorderReason" id="cancelReorderReason"/>
							<input	type="button" value="废 单 * 重下" class="right-button08"	onclick="cancelAndCreateNewOrder()" />
						</form>
					</s:if>
					<s:if test="orderDetail.orderStatus=='CANCEL'">
						<form action="<%=basePath%>/phoneOrder/index.do" method="post">
							<input type="hidden" name="orderId" value="${orderDetail.orderId}" />
							<input type="hidden" name="cancelType" value="toCreateOrderNew_new" />
							<input type="submit" value="订 单 * 重 下" class="right-button08" onclick="return confirm('你确定要订单重下吗？')"/></form>
						</s:if>
				</s:if>	
