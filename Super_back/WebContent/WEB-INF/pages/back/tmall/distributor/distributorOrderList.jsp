...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" src="../js/tmall/tmallDistributorOrder.js"></script>	
	...
	<body>
		...
		<div class="wapper_accounts">
			<div class="rad5 wapper_list wapper_list_cash">
				<h3 class="order_check">淘宝分销搬单列表</h3>
				<div class="cash_seach" >
					<form method="post" id="search_group_form" action="getOrderList.do">
						<ul class="order_top_list">
							<li class="other_list">
								<label>分销商：</label> 
								<input name="distributorUserName" type="text" class="input_text02 input_combox" 
									value="<s:property value="distributorUserName"/>"/>
							</li>
							<li class="other_list">
								<label>销售产品ID：</label> 
							...
							<li class="other_list">
								<label>分销流水号：</label> 
								<input name="fenXiaoId" type="text" class="input_text02 input_combox"  onkeyup="value=value.replace(/[^\d]/g,'')"
									value="<s:property value="fenXiaoId"/>"/>
							</li>
							<li class="other_list">
								<label>类别：</label>
								<s:select 
							...
							<li class="other_list">
							   <label>下单日期：</label>
							   <input id="createTimeStart" name="createTimeStart" type="text" class="date" onClick="WdatePicker()" value="<s:property value="createTimeStart"/>" />
							</li>
							<li class="other_list">
							   <label>采购单编号：</label>
							   <input id="distributorNo" name="distributorNo" type="text" class="input_text02 input_combox"  onkeyup="value=value.replace(/[^\d]/g,'')"
									value="<s:property value="distributorNo"/>"/>
							</li>
							<li class="">
							...
							<div class="rad5 tabw100 blue_skin_tb">
								<table cellspacing="0" cellpadding="0" border="0" class="rad5 tabw100 blue_skin_tb">
									...
									<s:iterator value="pagination.records" var="group" status="L">
										<tr>
										    <td width="25">${L.index+1}</td>
											<td width="90">${group.fenXiaoId}</td>
											<td width="105">${group.distributorNo}</td>
											<td width="110">${group.distributorUserName}</td>
											<td >${group.title}</td>
									        <td width="50">
									        ...
									        <td width="80">${group.lvOrderId}</td>
											<td width="50">
											...
											<td width="60" id="processStatus_${group.ordTmallDistributorMapId}">
											   <s:if test="#group.status=='success'">
										            <a  class="budget_see"  href="javascript:showLogFn('${group.ordTmallDistributorMapId }');">已处理</a> 
								              </s:if>
								              <s:elseif test="#group.status=='failure'">
								                  <a  class="budget_see"  href="javascript:manualProcessFn('${group.ordTmallDistributorMapId }');">人工下单</a>      
								              </s:elseif>
								              <s:elseif test="#group.status=='unpay'">
								                  <a  class="budget_see"  href="javascript:updateProcessStatus('${group.ordTmallDistributorMapId }','success',0);">处理支付</a>      
								              </s:elseif>
