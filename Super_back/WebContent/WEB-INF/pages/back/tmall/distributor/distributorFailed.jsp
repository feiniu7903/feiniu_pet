	...
	<body>
		...
		<div class="wapper_accounts">
			<div class="rad5 wapper_list wapper_list_cash">
				<h3 class="order_check">淘宝分销搬单失败列表</h3>
								...
								<table cellspacing="0" cellpadding="0" border="0" class="rad5 tabw100 blue_skin_tb">
									...
									<s:iterator value="pagination.records" var="group" status="L">
										<tr>
											...
											<td width="90">${group.distributorNo}</td>
											<td width="90">${group.distributorUserName}</td>
											<td width="15%">${group.title}</td>
											<td width="35%">${group.failedReason}</td>
											<td width="40">
											  <s:if test="#group.status=='failure'">未下单</s:if>
								              <s:elseif test="#group.status=='unpay'">未支付</s:elseif>
											</td>
											<td width="40" id="processStatusX_${group.ordTmallDistributorMapId}">
											  <s:if test="#group.status=='success'">
										            <a  class="budget_see"  href="javascript:void(0);">已处理</a> 
								              </s:if>
								              <s:elseif test="#group.status=='failure'">
								                  <a  class="budget_see"  href="javascript:updateProcessStatus('${group.ordTmallDistributorMapId }','create',1);">重新下单</a>      
								              </s:elseif>
								               <s:elseif test="#group.status=='unpay'">
								                  <a  class="budget_see"  href="javascript:updateProcessStatus('${group.ordTmallDistributorMapId }','success',1);">线下支付</a>      
								              </s:elseif>
											</td>
										</tr>
									</s:iterator>
