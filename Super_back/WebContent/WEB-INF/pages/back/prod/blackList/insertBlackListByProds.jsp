...
<div class="p_box" style="width: 650px;">
	...
	<script type="text/javascript">
		...
		function validateDate(){
			...
			}else if($("input[name='prodBlackList.blackStartTime']").val() == ""){
				flag = false;
				alert("请选择开始时间!");
			}else if($("input[name='prodBlackList.blackEndTime']").val() == ""){
				flag = false;
				alert("请选择结束时间!");
			}else if(saleCompareDate($("input[name='prodBlackList.blackStartTime']").val(),$("input[name='prodBlackList.blackEndTime']").val())){
				flag =false;
				alert("开始时间大于结束时间");
			}else if($("input[name='prodBlackList.blackLimit']").val() == ""){
				flag = false;
				alert("请输入限制笔数!");
			}else if($("textarea[name='prodBlackList.blackReason']").val() == ""){
				flag = false;
				alert("请输入限制原因!");
			}
			if(flag){
