...
<html>
	...
	<body>
		<a href="/super_back/tableEditorFromPlace.do">Switch to Fromplace Editor</a>
		<form id="form2" method="post" action="/super_back/tableEditorQuery.do">
		...
		<form id="form1" method="post" action="/super_back/tableEditor.do" onsubmit="return check();">
		...
		function setIpLocationId(obj) {
			...
			if (obj1.val()=='DZMP_RECOMMEND') {
				$.ajax({
					type: 'POST',
					url: '/super_back/tableEditor/ajaxGetIpLocationId.do',
		...
		function setToPlaceId(obj) {
			$.ajax({
				type: 'POST',
				url: '/super_back/tableEditor/ajaxGetToPlaceId.do',
				data: {placeName:obj.val()},
				dataType: 'text',
				success: function(data) {
		...
		$("#queryMore").bind("click", function(){
		  $("#form2").attr("action", "/super_back/tableEditorQueryMore.do");
		  $("#form2").submit();
		});
		</script>
	</body>
