...
$(function(){    
	$("input.findSameSupplierMetas").click(function(){
		var v=$(this).attr("checked");		
		var select=$(this).data("select");
		if(v&&(select=='undefined'||select==undefined)){
			$(this).data("select","true");
			var ordMetaId=$(this).val();
			var supplierId=$(this).attr("supplierId");
			var params=$("#frmWaitOrder").serialize();
			if(params!=''){
				params+="&";
			}
			
			params+="supplierId="+supplierId;//根据该条件来是否读取的是指定供应商订单项
			params+="&excludeOrdMetaId="+ordMetaId;
			//alert(params);
			$.post($("#frmWaitOrder").attr("action"),params,function(dt){
				if($.trim(dt)!=''){
					$("#wait_audit_tr_"+ordMetaId).after(dt);							
					var $metaIds=$("#tags_content_1").find("input.findSameSupplierMetasResult");
					$.each($metaIds,function(i,n){
						var id=$(n).val();
						if(document.getElementById("wait_audit_tr_"+id)){
							$("#wait_audit_tr_"+id).remove();
						}
					});
