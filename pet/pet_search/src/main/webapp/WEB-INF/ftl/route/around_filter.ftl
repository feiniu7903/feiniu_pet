...
<div class="search-filter">
	<div class="filter-info clearfix"><span class="result-info">共找到<i>${pageConfig.totalResultSize}</i>条结果。</span>
    	<dl id="your-choices" class="your-choices">
            <#if searchvo.startPrice?? || searchvo.endPrice?? 
            		...
            		|| searchvo.hotelType?? || searchvo.visitDay??
    ...
    <#if cities_show || subjects_show  || scenicPlaceMap_show 
		    || hotelTypes_show || hotelTypes_show 
		    || visitDays_show || playFeatures_show>
    <ul id="tags-list" class="filter-tags">
    	       <#if cities_show>
	                <li>
	                    ...
	                    <span>包含地区：</span>
                    	<p>
	                    	<a rel="nofollow"  class="s-tag <#if !searchvo.city??>selected</#if>" href="${base_url}<@fp filter="${filterStr}" type="A"  remove=true/>.html">全部</a>
				...
				<#if hotelTypes_show>
	                <li>
	                    ...
	                    <span>酒店类型：</span>
	                    <p>
		                    <a rel="nofollow" class="s-tag <#if !searchvo.hotelType??>selected</#if>" href="#">全部</a>
		                    <#list hotelTypes?keys as key>
