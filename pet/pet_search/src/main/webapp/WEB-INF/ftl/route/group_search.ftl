...
<#assign base_url = "http://www.lvmama.com/search/group/${fromDest}-${keyword}"/>
<head>
<meta charset="utf-8">
<title>${pageTitle!"跟团游_驴妈妈旅游网"}</title>
<meta name="keywords" content="${pageKeyword!"跟团游_驴妈妈旅游网"}" />
<meta name="description" content="${pageDescription!"跟团游_驴妈妈旅游网"}" />
...
<div class="lv-crumbs wrap">
    <p><b>您当前所处的位置：</b><a href="http://www.lvmama.com">首页</a> &gt; <a href="http://www.lvmama.com/search/route/${fromDest}-${keyword}.html">${searchvo.fromDest}出发</a> &gt; <a href="${base_url}.html">跟团游</a> &gt; <#if searchvo.city??><a href="${base_url}-A${searchvo.city}.html">${searchvo.city}</a> &gt; </#if>${searchvo.keyword}</p>
</div><!-- // div .lv-crumbs -->

<#include "/WEB-INF/ftl/common/search_box.ftl"/>

<div class="ui-content wrap">
	<#assign info_item_selected_group="true" />
	<#include "/WEB-INF/ftl/common/search_aside.ftl" >
	
    <div class="search-main search-line">
    	<#include "/WEB-INF/ftl/common/nav.ftl">
        <#include "/WEB-INF/ftl/route/group_filter.ftl">
        <!-- 线路搜索列表\\ -->
		<div id="list" class="search-result-box search-line-list">
		    ...
		    <#if pageConfig.items?size gt 0>
            	<#include "/WEB-INF/ftl/route/group_list.ftl">
            <#else>
...
if(<@s.property value="pageConfig.items.size()"/> > 0){
     cmCreatePageviewTag("产品搜索结果-跟团游（可预订商品2）", "G0001","<@s.property value="searchvo.fromDest"  escape="false"/>-"+"<@s.property value="searchvo.keyword"  escape="false"/>", "<@s.property value="pageConfig.totalResultSize"  escape="false"/>");
     }else{
     	cmCreatePageviewTag("产品搜索结果-跟团游-搜索失败", "G0001","<@s.property value="searchvo.fromDest"  escape="false"/>-"+"<@s.property value="searchvo.keyword"  escape="false"/>", "0");
     }
</script>
</body>
</html>