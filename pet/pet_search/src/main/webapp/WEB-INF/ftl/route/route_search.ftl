...
<#assign base_url = "http://www.lvmama.com/search/route/${fromDest}-${keyword}"/>
<head>
<meta charset="utf-8">
<title>${pageTitle!"度假_驴妈妈旅游网"}</title>
<meta name="keywords" content="${pageKeyword!"度假_驴妈妈旅游网"}" />
<meta name="description" content="${pageDescription!"度假_驴妈妈旅游网"}" />
...
<div class="lv-crumbs wrap">
    <p><b>您当前所处的位置：</b><a href="http://www.lvmama.com">首页</a> &gt; <a href="http://www.lvmama.com/search/route/${fromDest}-${keyword}.html">${searchvo.fromDest}出发</a> &gt; <#if searchvo.city??><a href="${base_url}-A${searchvo.city}.html">${searchvo.city}</a> &gt; </#if>${searchvo.keyword}</p>
</div><!-- // div .lv-crumbs -->
...
<div class="ui-content wrap">
    ...
    <div class="search-main search-line">
        ...
        <#include "/WEB-INF/ftl/route/route_filter.ftl">
        <!-- 线路搜索列表\\ -->
		<div id="list" class="search-result-box search-line-list">
		    ...
		    <#if pageConfig.items?size gt 0>
            	<#include "/WEB-INF/ftl/route/route_list.ftl">
            <#else>
...
if(<@s.property value="pageConfig.items.size()"/> > 0){
     cmCreatePageviewTag("产品搜索结果-全部路线（可预订商品2）", "G0001","<@s.property value="searchvo.fromDest"  escape="false"/>-"+"<@s.property value="searchvo.keyword"  escape="false"/>", "<@s.property value="tc.route"  escape="false"/>");
     }else{
     	cmCreatePageviewTag("产品搜索结果-全部路线-搜索失败", "G0001","<@s.property value="searchvo.fromDest"  escape="false"/>-"+"<@s.property value="searchvo.keyword"  escape="false"/>", "0");
     }
</script>
</body>
</html>