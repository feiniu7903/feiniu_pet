...
import com.lvmama.search.lucene.query.NewFreelongQueryUtil;
 ...
 *
 */
@Service("freelongSearchService")
public class FreelongSearchServiceImpl  extends RouteSearchServiceImpl implements RouteSearchService {
	...
	public PageConfig<ProductBean> search(SearchVO sv, SORT... sorts) {
		...
		params.put(ProductDocument.LAND_TRAFFIC, routeSearchVO.getLandTraffic());
		params.put(ProductDocument.LAND_FEATURE, routeSearchVO.getLandFeature());
		...
		params.put(ProductDocument.SUB_PRODUCT_TYPE, "FREENESS_FOREIGN,FREENESS_LONG");
		...
		Query query = NewFreelongQueryUtil.getNewFreeLongQuery(params);
