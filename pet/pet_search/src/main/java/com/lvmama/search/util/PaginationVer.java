...
public class PaginationVer {
	...
	public PaginationVer() {
		super();
	}

	public PaginationVer(int pageSize, int totalPage, String url, int page) {
	...
	public static String pagination(int pageSize,int totalPage,String url,int page){
		PaginationVer pagination = new PaginationVer();
	...
	public void createTag(int type){
		try {
			...
			case 0:
				long endPage2 = 0;
				if (prevPage >= 1) {
					str += "<a href='javascript:void(0);' title=\"上一页\" onclick='verSetPage(" +prevPage + ")' class=\"PrevPage\" rel=\"nofollow\" >上一页</a>";
				}
				if (this.page != 1)
				str +="<a href='javascript:void(0);' title=\"到第一页\" class=\"PageLink\" onclick='verSetPage(" +1 + ")'  rel=\"nofollow\" >1</a>";
					...
					if (i > 0) {
						if (i == this.page) {
							...
							if (i != 1 && i != this.totalPage) {
								str +="<a href='javascript:void(0);'  class=\"PageLink\" title='第 " + i + " 页'  rel=\"nofollow\" onclick='verSetPage(" +i + ")'>" + i
										+ "</a>";
							}
						}
					}
				...
				if (this.page + 3 < this.totalPage)
					str +="<span class=\"PageMore\">...</span>";
				if (this.page != this.totalPage)
					str +="<a  href='javascript:void(0);' onclick='verSetPage(" +this.totalPage + ")'  class=\"PageLink\" title= '第 " + this.totalPage
							+ " 页'  rel=\"nofollow\">" + this.totalPage + "</a>";

				if (nextPage <= this.totalPage) {
					str += "<a href='javascript:void(0);' title=\"下一页\" class=\"NextPage PageLink PageLink_page\" onclick='verSetPage(" +nextPage + ")'  rel=\"nofollow\">下一页</a>";
				}
...
//			str += "</div>";
	...
	private String toPage(long page) {
		...
		if ("htm".equals(this.type)) {
			...
			url = this.action.replace("{p}", String.valueOf(page))+".html";// urlrewriter 重写，结尾增加".html"
		}
		url="";
		return url;
	}
