...
public class PlaceQuery {
	...
	public static Query getSellPriceQuery(String startPrice, String endPrice) {
		...
		if (StringUtils.isNotBlank(startPrice) && startPrice.matches("[1-9][0-9]*\\.[0-9]*|[1-9][0-9]*")) {
			...
			start = bigDecimal_s.multiply(new BigDecimal(100)).longValue();
		}
		...
		if (StringUtils.isNotBlank(endPrice) && endPrice.matches("[1-9][0-9]*\\.[0-9]*|[1-9][0-9]*")) {
			...
			end = bigDecimal_e.multiply(new BigDecimal(100)).longValue();
		}
		return NumericRangeQuery.newLongRange(PlaceDocument.SELL_PRICE_RANG,start , end, true, true);
	}
