...
public class IPADPushJob {
	...
	private void beginSend() throws Exception {
		...
		param.put("start", 0);
		param.put("end", 100);
		param.put("plaform", Constant.MOBILE_PLATFORM.IPAD.name());
		while (true) {
			...
			for (MobilePushSendTask mobilePushSendTask : mobilePushSendTaskList) {
				...
				if (mobilePushSendTask.isPush2Ipad()) {
					this.notificationPush2IPAD(mobilePushJogTask,
							mobilePushSendTask);
				}
