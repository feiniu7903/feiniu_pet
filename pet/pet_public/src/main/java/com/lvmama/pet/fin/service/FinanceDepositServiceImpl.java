 ...
 *
 */
@HessianService("financeDepositService")
@Service("financeDepositService")
public class FinanceDepositServiceImpl extends BaseService implements FinanceDepositService{
	...
	public void addDeposit(FinDeposit fincDeposit){
		...
		if(Constant.FOREGIFTS_KIND.CASH.toString().equals(fincDeposit.getKind())){
			// 修改押金
			financeDepositDAO.updateSupplierDepositAmount(fincDeposit.getSupplierId(), amount);
		}else if(Constant.FOREGIFTS_KIND.GUARANTEE.toString().equals(fincDeposit.getKind())){
			// 修改担保函
			financeDepositDAO.updateSupplierGuaranteeLimit(fincDeposit.getSupplierId(), amount);
		}else{
			throw new RuntimeException("添加押金的类型错误！");
		}

		// 产生流水记录
		financeDepositDAO.insertFincDeposit(fincDeposit);
	...
	public void updateSupplierCurrency(FinDeposit fincDeposit) {
		// 根据供应商id查询是否存在供应商信息
		FinSupplierMoney finSupplierMoney = financeSupplierMoneyDAO.searchBySupplierId(fincDeposit.getSupplierId());
		// 存在供应商信息时，更新押金币种
		if(null != finSupplierMoney){
			financeDepositDAO.updateSupplierCurrency(fincDeposit.getSupplierId(), fincDeposit.getCurrency());
		}
		// 不存在供应商信息时，新增供应商（供应商ID, 押金余额， 押金币种）
		else{
			financeSupplierMoneyDAO.insertForegift(fincDeposit);
		}
	}

	@Override
	public FinSupplierMoney searchSupplier(Long supplierId) {
		return financeDepositDAO.searchSupplier(supplierId);
	}
}
