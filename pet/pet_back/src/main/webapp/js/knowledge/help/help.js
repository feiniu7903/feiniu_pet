		...
		if(objectWindow == contentTypeWindow)//add content type
		{
			...
			$.ajax( {
				...
				success : function(data) {
					var d = eval(data);
					if (d) {
						var resultInfoQuesTypeHierarchy=$("#listBusinessTypeGrid").datagrid("getSelected");
						$.each(d.rows, function(index, value) {
							$("#businessTypeSelect").append(
									'<option value=' + value.typeId + '>'
											+ value.typeName + '</option>');							
						});
						if(typeof(resultInfoQuesTypeHierarchy)!=undefined){
							$("#businessTypeSelect").find("option[value="+resultInfoQuesTypeHierarchy.typeId+"]").attr("selected",true);
						}
					} else {
						alert("获取发生错误！");
					}
...
function openAddOrEditContentWindow(command) {
	...
	else//edit
	{
		...
		editor.html(resultInfoQuesTypeHierarchy.content);		
	}
...
function saveContent() {
	...
	$("#oneHelpInfoRowContent").val(editor.html());
	...
	if (typeGrid == '#listBusinessTypeGrid') {
		businessTypeGrid = $(typeGrid).datagrid( {
			...
			onClickRow : function(rowIndex, rowData){
				//alert(rowIndex+"    "+rowData.typeName+"   "+rowData.typeId);
				createTypeListGrid(rowData,"#listContentTypeGrid");
			},
			frozenColumns : [ [
		...
		if(typeof(queryParam) == "object"){
			parentTypeId = queryParam.typeId;
		}
		contentTypeGrid = $(typeGrid).datagrid( {
			...
			remoteSort : false,
			idField : 'typeId',
			queryParams : {"resultInfoQuesTypeHierarchy.parentTypeId":parentTypeId},
			frozenColumns : [ [
...
function convertTextStringToHtmlString(textString)
{
