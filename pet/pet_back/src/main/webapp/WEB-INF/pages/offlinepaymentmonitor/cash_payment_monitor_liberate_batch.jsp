...
<body>
	<form method="post" action="" id="liberateForm">
		<table class="cg_xx" width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>	
				<td>解款总金额: 
					<s:if test="paymentsAmountSum!=null">
					${paymentsAmountSum/100}
					</s:if>
					<s:else>
						0
					</s:else>
			...
			<tr>
				<input type="hidden" value="${paymentIds}" name="paymentIds"/>
				<input type="hidden" value="${objectIds}" name="objectIds"/>
				<td colspan="2" align="center"">
					<s:if test="findFlag!='true'">
					<input name="right_button08Submit" type="button" value="确定" class="right-button08" id="liberateBatchSubmit"/>
					</s:if>
...
$("#liberateBatchSubmit").click(function(){
	...
	$.ajax({
			type:"POST", 
			url:'${basePath}/offlinepaymentmonitor/cash_payment_monitor!liberateBatch.do' + '?random=' + Math.random(), 
