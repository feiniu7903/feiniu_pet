    ...
    $.ajax({
        ...
        success:function (response){                    
        	if (response.success == true) {
        		error_tip("#mobileOrEmail","该手机号尚未注册，请重新输入或<a href='/nsso/register/registering.do'>立即注册</a>");  
            } else {  
            	success_function();             
            }
        }
...
function validEmailField(success_function) {
        $.ajax({
            ...
            success: function(response) {
                if (response.success == true) {                        
                    error_tip("#mobileOrEmail","该邮箱尚未注册，请重新输入或<a href='/nsso/register/registering.do'>立即注册</a>");  
                    return false;
                } else {
                    success_function();     
                }
...
function validCodeField() { 
    ...
    $.ajax({
        ...
        success:function (response) {               
            if (response.success == true) {
                window.location.href="/nsso/findpass/sendResetEmailSucc.do?email=" + $('#mobileOrEmail').val();
            } else {
