...
<title>成都出发国庆出境游_十一成都出发出境游线路-驴妈妈旅游网</title>
<meta name="keywords" content="成都出发,国庆出境游,十一旅游"/>
<meta name="description" content=" 驴妈妈国庆出境游专题:选择聪明的旅行!我们为您提供成都出发国庆出境游,十一成都出发出境游线路及实时比价服务，是您预订旅行产品,跟团游及自由行的最佳选择!"/>
	...
	<div class="wrapbot">
		...
		<div class="citymenu">
			<ul class="cityitems clearfix">
				...
				<li class="now"><a href="javascript:void(0)" target="_self"><span>成都/重庆<b></b></span></a></li> 
		...
		<div class="cont">
			<div class="state">
				<ul class="stateitems clearfix">
					<@s.iterator value="map.get('${station}_cd')" status="st">
				...
				<div class="tabbox">
					...
					<@s.iterator value="map.get('${station}_cd')" status="st">
						<@s.if test="#st.isFirst()">
							<div class="theme">
								<!--三个主题-->
								<@s.iterator value="map.get('${station}_cd_${st.index + 1}')" status="sts">
									<div class="zhuti1">
										<div class="thetop">${title?if_exists}</div>
										<div class="thecen">
											<ul class="themelist">
												<@s.iterator value="map.get('${station}_cd_${st.index + 1}_${sts.index + 1}')">
													<li>
