<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <title>上海到美国旅游报价_上海到美国旅游多少钱-驴妈妈旅游网</title>
    <meta name="keywords" content="上海到美国,美国旅游报价,多少钱" />
    <meta name="description" content="驴妈妈美国旅游专题:选择聪明的旅行!我们为您提供上海到美国旅游报价实时服务及各种门票优惠多少钱查询，是您预订旅游产品跟团游及自由行的最佳选择!" />
    <link rel="stylesheet" href="http://pic.lvmama.com/styles/zt/meiguo/America.css">
    <link rel="stylesheet" href="http://www.lvmama.com/zt/000global/styles/zt_global.css" >
    <script src="http://pic.lvmama.com/js/jquery142.js"></script>
    <script src="http://pic.lvmama.com/js/zt/meiguo/America.js"></script>
    <base target="_blank">
    <!--[if IE 6]>
    <script type="text/javascript" src="http://pic.lvmama.com/js/zt/DD_belatedPNG.js"></script>
    <script>DD_belatedPNG.fix('.nav_list3 p,.nav_r ul,.sh_r_d,.sh_yh_yd');</script>
    <![endif]-->
</head>

<body>
    <!--专题公共头部START-->
    <script src="http://www.lvmama.com/zt/000global/js/ztTopMenu.js" type="text/javascript"></script>
    <!--专题公共头部END-->
    <div class="sh_main_all">
        <div class="main_banner"></div>
        <ul class="main_nav">
            <li class="nav_list1">
                <a target="_self" href="."></a>
            </li>
            <li class="nav_list2">
                <a target="_self" href="sh_sm.html"></a>
            </li>
            <li class="nav_list3 nav_list3_a">
                <a target="_self" href="sh_yh.html"></a>
                <p></p>
            </li>
            <!--<li class="nav_list4">
                <a target="_self" href="sh_dz.html"></a>
            </li>-->
        </ul>

        <!--哪优惠-->
        <div class="sh_chanpin">
            <@s.iterator value="map.get('${station}_sh_nyh')" status="st">
                <dl class="sh_yh_list">
                    <dt>
                        <a href="${url?if_exists}">
                            <img src="${imgUrl?if_exists}" width="335" height="168" alt="${title?if_exists}"></a>
                    </dt>
                    <dd>
                        <h3>
                            <a href="${url?if_exists}">${title?if_exists}</a>
                        </h3>
                        <p>
                            ★ ${bakWord1?if_exists}
                            <br>
                            ★ ${bakWord2?if_exists}
                            <br>
                            ★ ${bakWord3?if_exists}
                            <br></p>
                        <!--<a class="sh_yh_yd" href="${url?if_exists}"></a>
                        <span>
                            出发日期： <i>${bakWord4?if_exists}</i>
                            驴妈妈价： <i>￥ <b>${memberPrice?if_exists?replace(".0","")}</b>
                                起</i> 
                        </span>-->
                    </dd>
                </dl>
            </@s.iterator>
        </div>

        <!--活动进行时-->
        <div class="huodong_img">
            <div class="img_list">
                <ul>
                    <li>
                        <a href="http://www.lvmama.com/zt/lvyou/maerdaifu">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei8.jpg" width="284" height="135" alt=""></a>
                    </li>
                    <li>
                        <a href="http://www.lvmama.com/zt/lvyou/ouzhou">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei5.jpg" width="284" height="135" alt=""></a>
                    </li>
                    <li>
                        <a href="http://www.lvmama.com/zt/lvyou/riben_1">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei4.jpg" width="284" height="135" alt=""></a>
                    </li>
                    <li>
                        <a href="http://www.lvmama.com/zt/lvyou/dsn">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei6.jpg" width="284" height="135" alt=""></a>
                    </li>
                    <li>
                        <a href="http://www.lvmama.com/zt/lvyou/zdf">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei1.jpg" width="284" height="135" alt=""></a>
                    </li>
                    <li>
                        <a href="http://www.lvmama.com/zt/lvyou/qianzheng">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei7.jpg" width="284" height="135" alt=""></a>
                    </li>
                    <li>
                        <a href="http://www.lvmama.com/zt/lvyou/shuqi">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei2.jpg" width="284" height="135" alt=""></a>
                    </li>
                    <li>
                        <a href="http://www.lvmama.com/zt/abroad/">
                            <img src="http://pic.lvmama.com/img/zt/meiguo/mei3.jpg" width="284" height="135" alt=""></a>
                    </li>
                </ul>
            </div>
            <span class="zuo"></span>
            <span class="you"></span>
        </div>

    </div>

    <!--顶部出发地-->
    <div class="main_cfd">
        <dl>
            <dt>旅游出发地：</dt>
            <dd class="main_cfd_dd">
                <a target="_self" href=".">上海</a>
            </dd>
            <dd>
                <a target="_self" href="cd.html">成都</a>
            </dd>
            <dd>
                <a target="_self" href="bj.html">北京</a>
            </dd>
            <dd>
                <a target="_self" href="gz.html">广州</a>
            </dd>
        </dl>
    </div>
    <div class="nav_r">
        <ul>
            <li>
                <a target="_self" href="."></a>
            </li>
            <li>
                <a target="_self" href="sh_sm.html"></a>
            </li>
            <li class="nav_r_li_top">
                <a target="_self" href="sh_yh.html"></a>
            </li>
            <li id="dingbu"></li>
        </ul>
        <span></span>
    </div>
    <!--背景-->
    <div class="body_bg_t">
        <span></span>
    </div>

    <script src="http://www.lvmama.com/zt/000global/js/ztFooter.js" type="text/javascript"></script>
    <script type="text/javascript" src="http://pic.lvmama.com/js/common/losc.js"></script>

</body>
</html>