 ...
 *
 */
public class UnionpayPreQueryServiceImpl implements PaymentQueryService {
	/**
	 * LOG.
	 */
	private static final Logger LOG = Logger.getLogger(UnionpayPreQueryServiceImpl.class);
	...
	private String transType = PaymentConstant.getInstance().getProperty("CHINAPAY_PRE_TRANSTYPE_PAY");
	...
	public PaymentQueryReturnInfo paymentStateQuery(PayPayment info) {
		...
		PayPrePayment payPrePayment = payPaymentService.selectPrePaymentByPaymentId(info.getPaymentId());
		if(null==payPrePayment){
			this.transType = PaymentConstant.getInstance().getProperty("CHINAPAY_PRE_TRANSTYPE_PAY");
		}else{
			if(Constant.PAYMENT_PRE_STATUS.CREATE.name().equalsIgnoreCase(payPrePayment.getStatus())){
				this.transType = PaymentConstant.getInstance().getProperty("CHINAPAY_PRE_TRANSTYPE_PAY");
			}else{
				this.transType = PaymentConstant.getInstance().getProperty("CHINAPAY_PRE_TRANSTYPE_PAYSUCC");
			}
		}
