...
public class CashAccountPayAction extends PayAction {

	private static final Logger LOG = Logger.getLogger(CashAccountPayAction.class);
	 ...
	 * 前台存款账户支付.
	 */
	@Action("/pay/cashAccountValidateAndPay")
	public void pay() {
		...
		try {
			if(checkSignature()){
				...
				// 从现金账户支付 
				List<Long> paymentIds = cashAccountService.payFromCashAccount(uu.getId(), orderId,this.getBizType(), super.getAmount(),0L);
				// 发送支付成功消息
			...
			LOG.error("pay from cash account failed, order id: " + orderId + ", user id: " + userNo + ", amount: " + super.getAmount());
	...
	@Override
	String getGateway() {
		return Constant.PAYMENT_GATEWAY.CASH_ACCOUNT.name();
	}
