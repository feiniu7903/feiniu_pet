 ...
 *
 */
public class UnionpayRefundServiceImpl implements BankRefundmentService{
	...
	public BankReturnInfo refund(RefundmentToBankInfo info) {
		...
		String[] valueVo = new String[]{
				"",//acqCode   c
				PaymentConstant.getInstance().getProperty("UNIONPAY_REFUND_CALLBACK_URL"),//backEndUrl   m
				charset,//charset  m
				"",//commodityDiscount  o
				"",//commodityName    o
				"",//commodityQuantity   o
				...
				"",//defaultBankNumber    o
				"",//defaultPayType   o
				PaymentConstant.getInstance().getProperty("UNIONPAY_REFUND_CALLBACK_URL"),//frontEndUrl    m
...
//				"",
				"",//transTimeout  o
				PaymentConstant.getInstance().getProperty("CHINAPAY_PRE_TRANSTYPE_PAY_REFUND"),//transType   m
				"",//transferFee   0
				version//version  m
		};
		...
		if (res != null && !"".equals(res)) {
			...
			if(UnionUtil.checkSecurity(key, arr)){//验证签名
				...
				returninfo.setCodeInfo(PaymentErrorData.getInstance().getErrorMessage(Constant.PAYMENT_GATEWAY.UNIONPAY.name(), result));
				if("00".equals(result)){
