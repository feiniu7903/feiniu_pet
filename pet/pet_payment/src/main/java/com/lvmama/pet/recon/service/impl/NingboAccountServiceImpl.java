...
public class NingboAccountServiceImpl implements GatewayAccountService{
	...
	//下载后默认使用的网关CODE
	private static String GATEWAY = Constant.RECON_GW_TYPE.NING_BO_BANK.name();
	//处理我方网关时包括的网关CODE
	private static String GATEWAY_IN = "'" + Constant.RECON_GW_TYPE.NING_BO_BANK.name() + "'";
	//成功标志
	...
	private String recon_url=PaymentConstant.getInstance().getProperty("NING_BO_BANK_RECON_URL");
	/**
	 * 定时任务自动处理宁波银行账务
	 */
	@Override
	...
	public String processAccount(Date startDate, Date endDate,String reconStatus) {
		//宁波银行必须按照日进行下载账单，否则会造成数据量过大，连接超时
		List<Date> dateList=DateUtil.getDateList(startDate, endDate);
		for (Date date : dateList) {
			this.startDate=date;
			this.endDate=date;
			//先将网关+当天日期的老数据删除
			int rows=finReconBankStatementService.deleteOldData(GATEWAY_IN,date);
			log.info("delete bank_statement old data,rows:"+rows+",GATEWAY_IN:"+GATEWAY_IN+",date:"+DateUtil.formatDate(date, "yyyy-MM-dd"));
			//下载、解析、插入银行端源数据
	...
	private String parseData(){
		try {
			//封装数据
			String postData=NingboReconUtil.initRequestParamsMap(reqCustomerId,startDate,endDate,dnHead,dnTail);
			log.info("NingboBank Recon sendMessage="+postData);
			//下载数据
			String result=HttpsUtil.requestPostData(recon_url,postData,"text/html","GBK",600000,600000).getResponseString("GBK");
			log.info("NingboBank Recon result=" + result);
			//是否下载成功
			if(!NingboReconUtil.isDownloadSuccess(result)){
				String errorMsg=NingboReconUtil.getDownloadErrorMsg(result);
				log.info("Download Fail errorMsg="+errorMsg);
				return errorMsg;
			}
			//验签
			if(!NingboReconUtil.checkSignature(result)){
				return "checkSignature Fail";
			}
			NingboResponseBean ningboResponseBean=NingboReconUtil.parseResponseBean(result);
			//将解析完成的数据封装对象
			List<FinReconBankStatement> finReconBankStatementPayList=convertFinReconBankStatementPay(ningboResponseBean.getNingboPaymentReconVOList(),startDate);
			List<FinReconBankStatement> finReconBankStatementRefundList=convertFinReconBankStatementRefund(ningboResponseBean.getNingboRefundReconVOList(),startDate);
			//再将最新下载的数据保存
			finReconBankStatementService.insert(finReconBankStatementPayList);
			finReconBankStatementService.insert(finReconBankStatementRefundList);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return SUCCESS;
	}
	private List<FinReconBankStatement> convertFinReconBankStatementPay(List<NingboPaymentReconVO> ningboPaymentReconVOList,Date downloadTime){
		List<FinReconBankStatement> finReconBankStatementList=new ArrayList<FinReconBankStatement>(); 
		for (NingboPaymentReconVO ningboPaymentReconVO : ningboPaymentReconVOList) {
			...
			finReconBankStatement.setBankPaymentTradeNo(ningboPaymentReconVO.getOutTradeNo());
			finReconBankStatement.setBankGatewayTradeNo(ningboPaymentReconVO.getTradeNo());
			finReconBankStatement.setAmount(PriceUtil.convertToFen(ningboPaymentReconVO.getTotalFee()));
			finReconBankStatement.setTransactionTime(DateUtil.toDate(ningboPaymentReconVO.getReqTime(), UtilDate.dtLong));
			finReconBankStatement.setDownloadTime(downloadTime);
			finReconBankStatement.setCreateTime(new Date());
			finReconBankStatement.setMemo(ningboPaymentReconVO.getPaymethod());
			finReconBankStatementList.add(finReconBankStatement);
		}
		return finReconBankStatementList;
	}
	private List<FinReconBankStatement> convertFinReconBankStatementRefund(List<NingboRefundReconVO> ningboRefundReconVOList,Date downloadTime){
		List<FinReconBankStatement> finReconBankStatementList=new ArrayList<FinReconBankStatement>(); 
		for (NingboRefundReconVO ningboRefundReconVO : ningboRefundReconVOList) {
			FinReconBankStatement finReconBankStatement=new FinReconBankStatement();
			finReconBankStatement.setGateway(GATEWAY);
			finReconBankStatement.setTransactionType(Constant.TRANSACTION_TYPE.REFUNDMENT.name());
			finReconBankStatement.setBankPaymentTradeNo(ningboRefundReconVO.getOutOrderNo());
			finReconBankStatement.setBankGatewayTradeNo(ningboRefundReconVO.getTradeNo());
			finReconBankStatement.setOutAmount(PriceUtil.convertToFen(ningboRefundReconVO.getRefundFee()));
			finReconBankStatement.setTransactionTime(DateUtil.toDate(ningboRefundReconVO.getGmtRefund(), UtilDate.dtLong));
			finReconBankStatement.setDownloadTime(downloadTime);
			finReconBankStatement.setCreateTime(new Date());
			finReconBankStatementList.add(finReconBankStatement);
		}
	...
	private String recon(String reconStatus){
		...
		if(result){
			...
			for (FinReconBankStatement finReconBankStatement : finReconBankStatementList) {
				try {
					//宁波银行对账(如果reconStatus为空则处理免对账交易,否则不执行)
			...
			//宁波银行对账后 如还有未对账状态的数据,则直接更新为我方有,宁波银行无匹配
