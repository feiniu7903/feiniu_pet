...
import com.lvmama.pet.recon.utils.SandPosPayReconUtil;
import com.lvmama.pet.recon.utils.UnionpayParsePayData;
import com.lvmama.pet.vo.UnionPayResponseBean;

public class SandPosAccountServiceImpl implements GatewayAccountService{
	...
	//下载后默认使用的网关CODE
	private static String GATEWAY = Constant.RECON_GW_TYPE.SAND_POS.name();
	//处理我方网关时包括的网关CODE
	private static String GATEWAY_IN = "'" + Constant.RECON_GW_TYPE.SAND_POS.name() + "'";
	//成功标志
	...
	private String parseData(){
		...
		// 请求数据,将解析完成的数据封装对象
		List<FinReconBankStatement> finReconBankStatementList = SandPosPayReconUtil.queryReconData(startDate);
		if(finReconBankStatementList==null || finReconBankStatementList.size()==0){
			log.error("download date fail for SAND_POS");
	...
	private String recon(String reconStatus){
		...
		if(result){
			...
			//杉德POS机对账后 如还有未对账状态的数据,则直接更新为我方有,银联无匹配
