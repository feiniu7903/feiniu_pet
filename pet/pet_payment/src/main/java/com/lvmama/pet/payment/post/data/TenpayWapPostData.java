 ...
 * @author ZhangJie
 */
public class TenpayWapPostData implements PostData {
	 ...
	 * 接收财付通通知的URL
	 */
	private String notify_url=PaymentConstant.getInstance().getProperty("TENPAY_WAP_NOTIFY_URL");
	/**
	 * 接收财付通通知的前台地址URL
	 */
	private String callback_url=PaymentConstant.getInstance().getProperty("TENPAY_WAP_CALLBACK_URL");
	...
	public TenpayWapPostData(PayPayment payment,Map<String,Object> paramMap){
	...
	public String getRequestDate() throws UnsupportedEncodingException{
		...
		params.append("&desc="+URLEncoder.encode(this.desc,"UTF-8"));
		if(StringUtils.isNotBlank(this.purchaser_id)){
        ...
        params.append("&callback_url="+this.callback_url);
		...
		if(StringUtils.isNotBlank(this.time_expire)){
