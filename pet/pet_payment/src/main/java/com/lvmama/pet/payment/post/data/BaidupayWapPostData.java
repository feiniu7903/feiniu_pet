 ...
 * @author ZhangJie
 */
public class BaidupayWapPostData implements PostData {
	 ...
	 */
	private String returnUrl=PaymentConstant.getInstance().getProperty("BAIDUWAPPAY_RETURN_URL");
	
	/**
	 * 用户点击该URL可以返回到商户网站；该URL也可以起到通知支付结果的作用
	 */
	private String pageUrl=PaymentConstant.getInstance().getProperty("BAIDUWAPPAY_PAGE_URL");
	...
	/**
	 * 网银支付或银行网关支付时，默认银行的编码,如果pay_type是银行网关支付，则必须有值。
	 */
	...
	public BaidupayWapPostData(PayPayment payment,Map<String,Object> paramMap){
		...
		if(null==paramMap.get("goodsDesc")||StringUtils.isBlank((String)paramMap.get("goodsDesc"))){
			...
			this.spNo=PaymentConstant.getInstance().getProperty("BAIDUWAPPAY_SP_NO");
		}else{
			...
			if(StringUtils.isNotBlank(this.goodsDesc)&&("半价".equalsIgnoreCase(this.goodsDesc)||"直减".equalsIgnoreCase(this.goodsDesc))){
				this.spNo=PaymentConstant.getInstance().getProperty("BAIDUWAPPAY_ACTIVITIES_SP_NO");
	...
	public String getRequestDate() throws UnsupportedEncodingException{
		...
		if(StringUtils.isNotBlank(this.pageUrl)){
			params.append("&page_url="+URLEncoder.encode(this.pageUrl,"GBK"));
		}
		if(StringUtils.isNotBlank(this.payType)){
			params.append("&pay_type="+this.payType);
		}
		if(StringUtils.isNotBlank(this.bankNo)){
			params.append("&bank_no="+this.bankNo);
		}
		if(StringUtils.isNotBlank(this.expireTime)){
			params.append("&expire_time="+this.expireTime);
		}
		if(StringUtils.isNotBlank(this.spUno)){
			params.append("&sp_uno="+this.spUno);
		}
		if(StringUtils.isNotBlank(this.inputCharset)){
