...
<html>
	...
	<body>
		<a href="/pet_back/adminTableEditorFromPlace.do">Switch to Fromplace Editor</a>
		<form id="form2" method="post" action="/pet_back/adminTableEditorQuery.do">
		...
		<form id="form1" method="post" action="/pet_back/adminTableEditor.do">
		...
		function del(id) {
			if (confirm("Determine delete?")) {
				$("#form1").attr("action", "/pet_back/adminTableEditor/del.do");
				$("#delId").val(id);
				$("#form1").submit();
			}
		}
		...
		$("#queryMore").bind("click", function(){
          $("#form2").attr("action", "/pet_back/adminTableEditorQueryMore.do");
          $("#form2").submit();
        });
		</script>
	</body>
